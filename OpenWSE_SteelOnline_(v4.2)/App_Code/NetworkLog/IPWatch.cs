...
public class IPWatch
{
    ...
    public IPWatch(bool getvalues)
    {
        if (getvalues)
        {
            dataTable = dbCall.CallSelect("IPWatchList", "", null, "Attempts DESC");
        }
    }
    ...
    public void addItem(string IPAddress, int attempts, bool blocked)
    {
        ...
        query.Add(new DatabaseQuery("LastAttempt", DateTime.Now.ToString()));

        dbCall.CallInsert("IPWatchList", query);
    }
    ...
    public bool CheckIfBlocked(string ip)
    {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("IPWatchList", "Blocked", query);
    ...
    public bool CheckIfExists(string ip)
    {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("IPWatchList", "ID", query);
        foreach (Dictionary<string, string> row in dbSelect) {
            bl = true;
            break;
        }
    ...
    public void updateAttempts(string ip, int attempts)
    {
        ...
        updateQuery.Add(new DatabaseQuery("LastAttempt", DateTime.Now.ToString()));

        dbCall.CallUpdate("IPWatchList", updateQuery, query);
        ...
        if (attempts >= ss.AutoBlockIPCount)
        {
            ...
            if (!string.IsNullOrEmpty(user.Email))
            {
                ...
                ServerSettings.SendNewEmail(message, "<h1 style='color:#555'>IP Watch Alert</h1>", ServerSettings.SiteName + ": IP Watch Alert", messagebody.ToString());
            }
        }
    }
    ...
    {
        dbCall.CallDelete("IPWatchList", new List<DatabaseQuery>() { new DatabaseQuery("IPAddress", ip) });
    }
}