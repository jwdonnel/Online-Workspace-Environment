...
namespace OpenWSE_Tools.Notifications {
    ...
    public class UserNotificationMessages {
                ...
                || (appId == "adaefeb2-9ef2-4ffa-b6ca-c76fc2815d4f")) // Error Report Notification ID / eRequests 
        {
            ...
            if (canContinue) {
                ...
                foreach (string nId in nIds) {
                    string emailNotification = notifications.IsEmailNotificationEnabled(username, nId);
                ...
                if (!string.IsNullOrEmpty(coll.ID)) {
                    if (mailTo.To.Count > 0) {
                        if (string.IsNullOrEmpty(subject))
                            subject = ServerSettings.SiteName + ": " + coll.NotificationName;
                        ...
                        string urlUnsubscribe = requestUrl + "Default.aspx?UnregisterEmails=" + nId;
        ...
        public void getEntries(string sortDir = "DESC") {
            _messages.Clear();
            List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_UserNotificationMessages", "", new List<DatabaseQuery>() { new DatabaseQuery("UserName", username) }, "Date " + sortDir);
        ...
        public void deleteAllUserNotification() {
            if (dbCall.CallDelete("aspnet_UserNotificationMessages", new List<DatabaseQuery>() { new DatabaseQuery("UserName", username) })) {
                _uuf.addFlag(username, "workspace", "");
            }
        }

        public void deleteNotification(string id) {
            if (dbCall.CallDelete("aspnet_UserNotificationMessages", new List<DatabaseQuery>() { new DatabaseQuery("UserName", username), new DatabaseQuery("ID", id) })) {
                _uuf.addFlag(username, "workspace", "");
            }
        }

        public void deleteNotification_All() {
            dbCall.CallDelete("aspnet_UserNotificationMessages", null);
        }
    }
