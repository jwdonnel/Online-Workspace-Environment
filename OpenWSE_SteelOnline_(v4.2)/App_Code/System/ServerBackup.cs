...
public class ServerBackup {
    ...
    public const string EncryptPassword = "1234512345678976";
    ...
    public void BinarySerialize(DataTable dt) {
        try {
            ...
            d.SerializeFile(_coll, loc);
            d.EncryptFile(loc, encryptloc, EncryptPassword);
        }
        catch {
        }
    }
    ...
    private void BinaryDeserialize() {
        try {
            if (File.Exists(loc)) {
                var d = new DataEncryption();
                MemoryStream a = d.DecryptFile(loc, EncryptPassword);
