...
public class AutoBackupSystem {
    ...
    public static bool AutoBackupState = false;
    ...
    public void GetEntries() {
        if (dataTable == null) {
            ...
            dataTable.Clear();
            List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("AutoBackupSystem", "", null, "BackupDay ASC");
    ...
    public AutoBackupSystem_Coll GetEntry(string _id) {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("AutoBackupSystem", "", new List<DatabaseQuery>() { new DatabaseQuery("ID", _id) });
        foreach (Dictionary<string, string> row in dbSelect) {
            string id = row["ID"];
            string day = row["BackupDay"];
            string time = row["BackupTime"];
    ...
    public void deleteItem(string id) {
        dbCall.CallDelete("AutoBackupSystem", new List<DatabaseQuery>() { new DatabaseQuery("ID", id) });
    ...
    public void StartBackupSystem() {
        ...
        DoStuff myAction = new DoStuff(BackupSystemChecker);
        myAction.BeginInvoke(null, null);
    }

    public void StopBackupSystem() {
    ...
    private void BackupSystemChecker() {
        ...
        while (true) {
            if (AutoBackupState) {
                ...
                string timeofday = DateTime.Now.ToShortTimeString().ToLower();
    ...
    private void StartRunningBackup(string backupType) {
        ...
        switch (backupType.ToLower()) {
            case "partial":
                ...
                string loc = serverPath + "Backups\\Database_" + DateTime.Now.ToFileTime() + "Temp" + ServerSettings.BackupFileExt;
