...
namespace OpenWSE_Tools.Overlays {
    ...
    public class WorkspaceOverlays {
        ...
        public void GetWorkspaceOverlays() {
            ...
            List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("WorkspaceOverlays", "", null, "OverlayName ASC");
        ...
        public string GetWorkspaceOverlayID(string fileLoc) {
            DatabaseQuery dbSelect = dbCall.CallSelectSingle("WorkspaceOverlays", "ID", new List<DatabaseQuery>() { new DatabaseQuery("FileLoc", fileLoc) });
            return dbSelect.Value;
        }

        public string GetWorkspaceFileLoc(string id) {
            DatabaseQuery dbSelect = dbCall.CallSelectSingle("WorkspaceOverlays", "FileLoc", new List<DatabaseQuery>() { new DatabaseQuery("ID", id) });
            return dbSelect.Value;
        }

        public string GetWorkspaceOverlayDefaultPosition(string id) {
            DatabaseQuery dbSelect = dbCall.CallSelectSingle("WorkspaceOverlays", "Position", new List<DatabaseQuery>() { new DatabaseQuery("ID", id) });
            return dbSelect.Value;
        }
        ...
        public WorkspaceOverlay_Coll GetWorkspaceOverlay(string _id) {
            ...
            if (!string.IsNullOrEmpty(_id)) {
                List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("WorkspaceOverlays", "", new List<DatabaseQuery>() { new DatabaseQuery("ID", _id) });
                foreach (Dictionary<string, string> row in dbSelect) {
                    string id = row["ID"];
                    string on = row["OverlayName"];
                    string fl = row["FileLoc"];
        ...
        public WorkspaceOverlay_Coll GetWorkspaceOverlayByFileLoc(string fileLoc) {
            WorkspaceOverlay_Coll coll = new WorkspaceOverlay_Coll();
            List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("WorkspaceOverlays", "", new List<DatabaseQuery>() { new DatabaseQuery("FileLoc", fileLoc) });
            foreach (Dictionary<string, string> row in dbSelect) {
                string id = row["ID"];
                string on = row["OverlayName"];
                string fl = row["FileLoc"];
        ...
        public void DeleteOverlay(string id) {
            dbCall.CallDelete("WorkspaceOverlays", new List<DatabaseQuery>() { new DatabaseQuery("ID", id) });
        }
        ...
        public void GetUserOverlays(string userName) {
            _userOverlays.Clear();
            List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_UserOverlays", "OverlayID", new List<DatabaseQuery>() { new DatabaseQuery("UserName", userName.ToLower()) });
            foreach (Dictionary<string, string> row in dbSelect) {
        ...
        public bool IsUserOverlayEnabled(string username, string overlayId) {
            List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_UserOverlays", "", new List<DatabaseQuery>() { new DatabaseQuery("OverlayID", overlayId), new DatabaseQuery("UserName", username.ToLower()) });
            if (dbSelect.Count > 0) {
                return true;
            }
        ...
        public void DeleteUserOverlay(string username, string overlayId) {
            dbCall.CallDelete("aspnet_UserOverlays", new List<DatabaseQuery>() { new DatabaseQuery("OverlayID", overlayId), new DatabaseQuery("UserName", username.ToLower()) });
        }

        public void DeleteAllUserOverlays(string username) {
            dbCall.CallDelete("aspnet_UserOverlays", new List<DatabaseQuery>() { new DatabaseQuery("UserName", username.ToLower()) });
        }
