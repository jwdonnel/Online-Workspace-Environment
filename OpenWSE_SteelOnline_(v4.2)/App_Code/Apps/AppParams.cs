...
public class AppParams {
    ...
    public AppParams(bool getvalues) {
        if (getvalues) {
            dataTable = dbCall.CallSelect("AppParams", "", null, "Parameter ASC");
        }
    }
    ...
    public void GetAllParameters_ForApp(string appid) {
        dataTable = dbCall.CallSelect("AppParams", "", new List<DatabaseQuery>() { new DatabaseQuery("AppID", appid) }, "Parameter ASC");
    }

    public void updateParameter(string id, string appid, string parameter, string user) {
        List<DatabaseQuery> updateQuery = new List<DatabaseQuery>();
        updateQuery.Add(new DatabaseQuery("Parameter", parameter));
        updateQuery.Add(new DatabaseQuery("UserName", user));
        updateQuery.Add(new DatabaseQuery("DateTime", DateTime.Now.ToString()));

        dbCall.CallUpdate("AppParams", updateQuery, new List<DatabaseQuery>() { new DatabaseQuery("ID", id) });
    }

    public void deleteParameter(string id) {
        dbCall.CallDelete("AppParams", new List<DatabaseQuery>() { new DatabaseQuery("ID", id) });
    }
