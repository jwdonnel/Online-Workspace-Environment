...
public class SiteMessageBoard {
    ...
    public void getEntriesByGroup(string groupname) {
        dataTable = dbCall.CallSelect("MessageBoard", "", new List<DatabaseQuery>() { new DatabaseQuery("GroupName", groupname) }, "Date DESC");
    }
    ...
    public void getEntries(string sortDir = "DESC") {
        ...
        dataTable = dbCall.CallSelect("MessageBoard", "", null, "Date " + sortDir);
    }

    public string getGroupName_byMessageID(string id) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("MessageBoard", "GroupName", new List<DatabaseQuery>() { new DatabaseQuery("ID", id) });
    ...
    public void deletePost(Guid id, string groupname) {
        bool setFlags = false;
        if (dbCall.CallDelete("MessageBoard", new List<DatabaseQuery>() { new DatabaseQuery("UserName", username), new DatabaseQuery("ID", id.ToString()) })) {
            setFlags = true;
        }
    ...
    public void deleteUserPosts(string username) {
        dbCall.CallDelete("MessageBoard", new List<DatabaseQuery>() { new DatabaseQuery("UserName", username) });
    }
