...
public partial class SiteSettings_dbViewer : System.Web.UI.Page {
    ...
    private void BuildDropDown(DataTable dt) {
        ...
        try {
            ...
            foreach (var dbcoll in db.DBColl) {
                if (AppExists_Import(dbcoll.ID)) {
                    if ((_username.ToLower() != dbcoll.ImportedBy.ToLower()) && (apps.GetIsPrivate(dbcoll.ID))) {
                        continue;
                    }
                }

                if (!dbcoll.TableName.ToLower().Contains("aspnet_") && !dbcoll.TableName.ToLower().Contains("membership")) {
                    var item = new ListItem(dbcoll.TableName + " (Imported)", dbcoll.TableName + "(Imported)");
            ...
            foreach (DataRow dr in dt.Rows) {
                if (!dr["TABLE_NAME"].ToString().ToLower().Contains("aspnet_") && !dr["TABLE_NAME"].ToString().ToLower().Contains("membership")) {
                    ...
                    if (dr["TABLE_NAME"].ToString().IndexOf("CT_") == 0) {
                        CustomTableViewer ctv = new CustomTableViewer(_username);
                        CustomTable_Coll cInfo = ctv.GetTableInfo(tableName);

                        if (AppExists_Custom(cInfo.AppID)) {
                            if ((_username.ToLower() != cInfo.CreatedBy.ToLower()) && (apps.GetIsPrivate(cInfo.AppID))) {
                                continue;
                            }
                        }

                        tableName = cInfo.TableName;
                ...
                if (tablename.Contains("(Imported)")) {
                    ...
                    foreach (
                        var dbcoll in
                            db.DBColl.Where(
                                dbcoll =>
                                dbcoll.TableName.ToLower() == tablename.Replace("(Imported)", "").Trim().ToLower())) {
                        dbcollTemp = dbcoll;
                        found = true;
                        break;
                    }
    ...
    private void BuildTableSchema(DataTable dt, string tableName) {
        ...
        if (dt.Columns.Count > 0) {
            ...
            str.Append("<tr class='myHeaderStyle'><td width='40px'></td><td style='min-width: 110px;'>Column Name</td><td style='min-width: 115px;'>Type</td><td>Nullable</td></tr>");
    ...
    private void StartRunningBackup_Ind() {
        ...
        if (currTable.IndexOf("CT_") == 0) {
            string tempTablename = currTable;
            CustomTableViewer ctv = new CustomTableViewer(_username);
            currTable = ctv.GetTableNameByTableID(currTable).Replace(" ", "_");
