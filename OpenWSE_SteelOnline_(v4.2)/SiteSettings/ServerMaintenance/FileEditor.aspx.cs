...
public partial class SiteSettings_FileEditor : Page {
        ...
        if (!userId.IsAuthenticated) {
            ...
            if (ServerSettings.AdminPagesCheck(Page.ToString(), userId.Name)) {
                if (HelperMethods.ConvertBitToBoolean(Request.QueryString["toolView"])) {
                    lbtn_close.NavigateUrl = "FileEditor.aspx?toolView=true";
                }

                if (!IsPostBack) {
                    dd_viewtype.SelectedIndex = 0;
                    if ((userId.Name.ToLower() != ServerSettings.AdminUserName.ToLower()) && (_ss.LockFileEditor)) {
                    ...
                    if (!string.IsNullOrEmpty(Request.QueryString["edit"])) {
                        ...
                        if (HelperMethods.ConvertBitToBoolean((Request.QueryString["edit"])) && ((userId.Name.ToLower() == ServerSettings.AdminUserName.ToLower()) || (!_ss.LockFileEditor))) {
                            pnl1.Enabled = false;
                            pnl1.Visible = false;
                            pnl2.Enabled = true;
                            pnl2.Visible = true;
    ...
    private void GetCallBack() {
        ...
        switch (controlName) {
            case "MainContent_lbtn_save":
                ...
                if ((HttpContext.Current.User.Identity.Name.ToLower() == ServerSettings.AdminUserName.ToLower()) || (!_ss.LockFileEditor)) {
                    if ((fi.Directory != null) && (fi.Extension.ToLower() == ".css") && (fi.Directory.Name.ToLower() != "standard")) {
                        ...
                        ServerSettings.PageToolViewRedirect(this.Page, "FileEditor.aspx");
                    }
                    else {
