...
$(document).ready(function () {
    ResizeSideBar();
    loadFontSize();
    __doPostBack('btn_modalClose_deliverypickups', '');
});
...
$(document.body).on("change", "#font-size-selector", function() {
    ...
    cookie.set("deliverypickups-fontsize", fontsize, "30");
});

function loadFontSize() {
    var fontsize = cookie.get("deliverypickups-fontsize");
...
$(document.body).on("click", ".GridDatePrev", function() {
    try {
        ...
        hf_d.value = "";
        hf_d.value = curr;
        __doPostBack("hf_refreshTimer_deliverypickups", "");
    } catch(evt) {
...
$(document.body).on("click", ".catButtons", function() {
    ...
    if (hf_category.value != t) {
        hf_category.value = t;
        __doPostBack("hf_category_deliverypickups", "");
    } else {
        hf_category.value = "";
        __doPostBack("hf_category_deliverypickups", "");
    }
...
prm.add_endRequest(function() {
    ...
    $("#tb_search_deliverypickups").autocomplete({
        ...
        source: function(request, response) {
            $.ajax({
                url: openWSE.siteRoot() + "WebServices/AutoComplete_Custom.asmx/GetSchedule",
