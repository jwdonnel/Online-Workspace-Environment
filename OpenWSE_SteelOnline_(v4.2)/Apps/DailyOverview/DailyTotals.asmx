    ...
    public string GetTotals() 
    {
        ...
        if (HttpContext.Current.User.Identity.IsAuthenticated) {
            ...
            foreach (System.Collections.Generic.Dictionary<string, string> row in gd.generaldirection) {
                str.Append("<b class='pad-right'>" + row["GeneralDirection"].ToString() + ":</b>" + (ts.calTotalWeightGD(DateTime.Now.ToShortDateString(), row["GeneralDirection"].ToString()).ToString("#,##0")) + " lbs<br />");
                totalweight += Convert.ToInt32(ts.calTotalWeightGD(DateTime.Now.ToShortDateString(), row["GeneralDirection"].ToString()));
            }
