...
public class WorkCalendar : WebService
{
        ...
        if (userID.IsAuthenticated)
        {
            try
            {
                ...
                if (ce.ce_dt.Count == 1)
                {
                    ...
                    string usercolor = UserImageColorCreator.CreateImgColor(m.AccountImage, color, m.UserId, 35);
                            
                    str.Append("<div class='float-left pad-right'><img alt='' src='App_Themes/" + _sitetheme + "/App/plan.png' style='height: 35px;' /></div>");
                        ...
                        (DateTime.TryParse(dr["EndDate"].ToString(), out date2)))
                    {
                        ...
                        {
                            if (m.Username.ToLower() == userID.Name.ToLower())
                            {
    ...
    public object[] GetUserListForGroup(string group)
    {
        ...
        if (IsApprover(userID.Name)) {
            ...
            MembershipUserCollection coll = Membership.GetAllUsers();
            foreach (var item in from MembershipUser u in coll let apps = new App() let member = new MemberDatabase(u.UserName) let g = new Groups(u.UserName) where member.UserHasApp("app-workcalendar") where g.IsApartOfGroup(member.GroupList, @group) select new System.Web.UI.WebControls.ListItem(HelperMethods.MergeFMLNames(member), u.UserName) into item where !items.Contains(item) select item) {
            ...
            foreach (Dictionary<string, string> dr in ce.ce_dt)
            {
                    ...
                    (DateTime.TryParse(dr["EndDate"].ToString(), out date2)))
                {
                    if (!HelperMethods.ConvertBitToBoolean(countOnly))
                    {
                        ...
                        string userColor = UserImageColorCreator.CreateImgColor(mb.AccountImage, mb.UserColor, mb.UserId, 35);
        ...
        foreach (Dictionary<string, string> dr in _ce.ce_dt.Cast<Dictionary<string, string>>().Where(dr => dr["GroupName"].ToString() == group))
        {
            ...
            if (HelperMethods.ConvertBitToBoolean(dr["Approved"]))
            {
                    ...
                    (DateTime.TryParse(dr["EndDate"].ToString(), out date2)))
                {
                    ...
                    int numDaysInMonth = DateTime.DaysInMonth(DateTime.Now.Year, DateTime.Now.Month);
                    if (dayOfMonth < numDaysInMonth)
                        dayOfMonth = DateTime.Now.Day + 1;
