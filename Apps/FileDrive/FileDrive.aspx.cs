...
public partial class Apps_FileDrive : Page {
    ...
    protected void Page_Load(object sender, EventArgs e) {
        ...
        if (_appInitializer.TryLoadPageEvent) {
            ...
            if (GroupSessions.DoesUserHaveGroupLoginSessionKey(_username)) {
                ...
                hf_ddgroups.Value = GroupSessions.GetUserGroupSessionName(_username);
            }
            else {
    ...
    private void LoadGroups() {
        if (!GroupSessions.DoesUserHaveGroupLoginSessionKey(_username)) {
            var itemNone = new ListItem("- Public -", string.Empty);
    ...
    private void LoadDocuments() {
        ...
        string countPersonal = docsPersonal.documents_coll.Count.ToString();
        ...
        str.Append("<div class='clear-margin'><input id='tb_newfolderentry' type='text' maxlength='15' value='Create New Folder' class='textEntry margin-right' style='width: 150px;' ");
        str.Append("onfocus=\"if(this.value=='Create New Folder')this.value=''\" onblur=\"if(this.value=='')this.value='Create New Folder'\" />");
        str.Append("<input type='button' onclick='createnewfolder();return false;' class='input-buttons no-margin' value='Create' /><br />");
        ...
        if (string.IsNullOrEmpty(hf_ddgroups.Value)) {
            int newCount = 0;
            foreach (FileDriveDocuments_Coll fdColl in docsPersonal.documents_coll) {
                if (fdColl.GroupName == hf_ddgroups.Value) {
                    newCount++;
                }
            }

            countPersonal = newCount.ToString();
        }

        str.Append("<div id='expand_View_All_Documents' class='tsdiv RandomActionBtns-docs" + addCssStart + "' onclick='folderChange(\"0\")'><span class='first-node float-left' style='padding: 3px'>View All Files in Group</span></div>");
        ...
        str.Append("<div class='sidebar-divider'></div><div class='clear-space'></div>");
        if (docs.folders_coll.Count == 0) {
            str.Append("There are no folders in group");
        }
        else {
            int count = 0;
            List<string> groupList = hf_ddgroups.Value.Split(ServerSettings.StringDelimiter_Array, StringSplitOptions.RemoveEmptyEntries).ToList();
            ...
            foreach (var t in docs.folders_coll) {
                foreach (string gr in groupList) {
                    ...
                    smwtemp.GetFilesByFolderName(t.ID.ToString(), gr);
    ...
    protected void GV_Files_RowCreated(object sender, GridViewRowEventArgs e) {
        if (e.Row.RowType == DataControlRowType.Pager) {
            ...
            if (e.Row.Cells.Count > 0) {
                Label GV_Files_documents_lbl_album = (Label)e.Row.Cells[0].FindControl("lbl_album");
                if (GV_Files_documents_lbl_album != null) {
                    GV_Files_documents_lbl_album.Text = "Folder/Group";
                    lbl_movetofolder_documents.Text = "Move Selected Files to a Folder/Group:";
                }
            }
        }
    }
    ...
    protected void GV_Files_RowEdit(object sender, GridViewEditEventArgs e) {
        ...
        var dd_editFolderName = (CheckBoxList)GV_Files_documents.Rows[e.NewEditIndex].FindControl("dd_editFolderName");
        var dd_editFolderName1 = (DropDownList)GV_Files_documents.Rows[e.NewEditIndex].FindControl("dd_editFolderName1");
        var hf_editFolderName = (HiddenField)GV_Files_documents.Rows[e.NewEditIndex].FindControl("hf_editFolderName");
        var hf_editGroupList = (HiddenField)GV_Files_documents.Rows[e.NewEditIndex].FindControl("hf_editGroupList");

        docs.GetAllFolders(hf_ddgroups.Value);
        dd_editFolderName1.Items.Clear();
        var itemstart = new ListItem("Root Directory", "-");
        dd_editFolderName1.Items.Add(itemstart);
        dd_editFolderName1.SelectedIndex = 0;

        string memberId = member.UserId;
        var itemMyFolder = new ListItem("Personal Folder", memberId);
        dd_editFolderName1.Items.Add(itemMyFolder);

        if (hf_editFolderName.Value == memberId)
            dd_editFolderName1.SelectedIndex = 1;

        if (hf_editGroupList.Value.Split(ServerSettings.StringDelimiter_Array, StringSplitOptions.None).ToList().Count == 1) {
            int count = 2;
            foreach (FileDriveFolders_Coll t in docs.folders_coll) {
                if (t.FolderName != memberId) {
                    var item = new ListItem(GetAlbum(t.FolderName), t.ID.ToString());
                    if (!dd_editFolderName1.Items.Contains(item)) {
                        dd_editFolderName1.Items.Add(item);
                        if ((item.Value == hf_editFolderName.Value) || (item.Value == hf_editFolderName.Value.Replace(" ", "_")))
                            dd_editFolderName1.SelectedIndex = count;
        ...
        dd_editFolderName.Items.Add(new ListItem("- Public -", ""));

        List<string> fileGroupList = hf_editGroupList.Value.Split(ServerSettings.StringDelimiter_Array, StringSplitOptions.None).ToList();

        foreach (string f in fileGroupList) {
            if (string.IsNullOrEmpty(f)) {
                dd_editFolderName.Items[0].Selected = true;
                break;
            }
        }
        ...
        foreach (string g in userGroups) {
            ...
            var itemstartGroupName = new ListItem(groupName, g);
            dd_editFolderName.Items.Add(itemstartGroupName);
            if (fileGroupList.Contains(g)) {
                dd_editFolderName.Items[dd_editFolderName.Items.Count - 1].Selected = true;
            }
        }

        RegisterPostbackScripts.RegisterStartupScript(this, "$('#GV_Files_documents_td_album').show();");
    }
    ...
    protected void GV_Files_RowUpdate(object sender, GridViewUpdateEventArgs e) {
        ...
        var ddEditFolderName = (CheckBoxList)GV_Files_documents.Rows[e.RowIndex].FindControl("dd_editFolderName");
        var ddEditFolderName1 = (DropDownList)GV_Files_documents.Rows[e.RowIndex].FindControl("dd_editFolderName1");
        ...
        int totalCount = 0;
        string list = "";
        foreach (ListItem item in ddEditFolderName.Items) {
            if (item.Selected) {
                list += item.Value + ServerSettings.StringDelimiter;
                totalCount++;
            }
        }

        if (totalCount > 1) {
            docs.updateFolderName(Guid.Parse(id.Value), "-");
        }
        else {
            docs.updateFolderName(Guid.Parse(id.Value), ddEditFolderName1.SelectedValue);
        }

        if (list.Length > 0 && list[list.Length - 1].ToString() == ServerSettings.StringDelimiter) {
            list = list.Remove(list.Length - 1);
        }

        docs.updateDocumentGroup(id.Value, list);
    ...
    public void LoadFiles(ref GridView gv, string sortExp, string sortDir) {
        ...
        List<FileDriveDocuments_Coll> tempColl = docs.documents_coll;
        if ((!string.IsNullOrEmpty(tb_search_documents.Text)) && (tb_search_documents.Text != "Search Files")) {
            docs.GetAllFiles(hf_ddgroups.Value);
        ...
        DataView dvFiles = GetFiles(tempColl, personalFolder);
        if (dvFiles.Count > 0) {
    ...
    public DataView GetFiles(List<FileDriveDocuments_Coll> file_coll, bool personalFolder) {
        ...
        dtFiles.Columns.Add(new DataColumn("GroupList"));
        ...
        foreach (var x in file_coll) {
            ...
            string[] groupList = hf_ddgroups.Value.Split(ServerSettings.StringDelimiter_Array, StringSplitOptions.None);
            List<string> fileGroupList = x.GroupName.Split(ServerSettings.StringDelimiter_Array, StringSplitOptions.None).ToList();
            ...
            if (isInGroup) {
                ...
                if (IsImageFileType(x.FileExtension)) {
                    if ((ViewState["HideImages"] != null) && ((bool)ViewState["HideImages"] == true)) {
                        ...
                        string imgLink = "<a href='" + path + x.FileExtension + "' target='_blank' class='cursor-pointer' title='View Image'><img alt='preview' src='" + path + x.FileExtension + "' style='max-height: 100px; max-width: 100%;' /></a><div class='clear-space-five'></div>";
                    ...
                    string fn = "<div class='float-left audio-file' data-src='" + path + x.FileExtension + "'>" + fileTitle + "</div>";
                    fileTitle = fn + "<img id='" + x.ID + "' alt='Play_Stop' src='" + imagePath + "' title='Play/Stop' class='margin-left float-left cursor-pointer audio' " + onClick + " />";
                }
                ...
                drFile["Album"] = x.Folder;
                drFile["GroupList"] = x.GroupName;
        ...
        RegisterPostbackScripts.RegisterStartupScript(this, "$('#GV_Files_documents_td_album').hide();");
    ...
    private void ResetSelected() {
        ...
        dd_moveFolder_documents.Enabled = false;
        dd_moveFolder_documents.Visible = false;
    ...
    protected void CheckBoxIndv_CheckChanged(object sender, EventArgs e) {
        ...
        if (!string.IsNullOrEmpty(HiddenField1_documents.Value)) {
            ...
            dd_moveFolder_documents.Enabled = true;
            dd_moveFolder_documents.Visible = true;
            ...
            docs.GetAllFolders(hf_ddgroups.Value);
            BuildFolderDropDowns(dd_moveFolder_documents);

            dd_myAlbums_documents.Items.Add(new ListItem("- Public -", string.Empty));
    ...
    protected void lbtn_moveFile_Click(object sender, EventArgs e) {
        ...
        for (int i = 0; i < filelist.Length; i++) {
            docs.updateFolderName(Guid.Parse(filelist[i]), dd_moveFolder_documents.SelectedValue);
            docs.updateDocumentGroup(filelist[i], dd_myAlbums_documents.SelectedValue);
        }
    ...
    private void GetNewFileNotification() {
        int count =
            (from t in docs.documents_coll let now = ServerSettings.ServerDateTime select now.Subtract(t.UploadDate)).Count(
                final => final.Days <= 1);
        ...
        if (FileUploadControl.HasFile) {
            try {
                ...
                foreach (HttpPostedFile file in FileUploadControl.PostedFiles) {
                    ...
                    string folder = hf_category_documents.Value;
                    ...
                    var info = new FileInfo(p);
                    if (info.Exists) {
                        filesql.addFile(fileNameId, fileNamesql, tempext, HelperMethods.FormatBytes(info.Length), pathsql, string.Empty, folder, groupname, false);
                    }
                }
            }
            catch (Exception ex) {
