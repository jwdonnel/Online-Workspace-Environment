...
public partial class SiteTools_NotifiManager : System.Web.UI.Page {
    ...
    private void BuildNotifi() {
                    ...
                    if (Roles.IsUserInRole(_username, ServerSettings.AdminUserName) || coll.UserName.ToLower() == _username.ToLower()) {
                        ...
                        if (!isEditMode) {
                            ...
                            if (string.IsNullOrEmpty(hf_EditNotifi.Value)) {
                                ...
                                str.Append("<td width='75px' align='center' class='border-right border-bottom'><div class='float-left' style='height: 22px;'></div></td></tr>");
                            }
                        }
                        else {
        ...
        if (showRefresh && Roles.IsUserInRole(_username, ServerSettings.AdminUserName)) {
            if (string.IsNullOrEmpty(tb_search.Text.Trim().ToLower()) || tb_search.Text.Trim().ToLower() == tb_search.Attributes["data-defaultvalue"].ToLower()) {
        ...
        if (count == 0)
            str.Append("<div class='emptyGridView'>No Notifications Found</div>");
    ...
    protected void lbtn_Refresh_Clicked(object sender, EventArgs e) {
        ...
        if (string.IsNullOrEmpty(_notifications.GetNotification("707ecc6c-2480-4080-bad6-fb135bb5cf13").ID))
            _notifications.AddNotification(_username, "Table Updates", "~/Standard_Images/Notifications/database.png", "Alert users when a custom table or table import has changed. (Only works for users that have a custom table or database import app installed)", "707ecc6c-2480-4080-bad6-fb135bb5cf13");
    ...
    private void LoadAppIconsEdit(string id) {
        ...
        if (!string.IsNullOrEmpty(tempDob.ID)) {
            ...
            foreach (Apps_Coll dr in tempAppColl) {
                ...
                if (nIds.Length == 0) {
                    if (!removeTemp.ContainsKey(_appId)) {
                        ...
                        appScript.Append("<div id='app-icon-" + _appId + "' class='app-icon-admin inline-block'>" + image);
            ...
            string table = HelperMethods.TableAddRemove(removeScript.ToString(), addScript.ToString(), "App Available to Add", "Associated Apps", true);
    ...
    private bool SearchFilterValid(Notifications_Coll nc) {
        ...
        if (string.IsNullOrEmpty(searchText) || searchText == tb_search.Attributes["data-defaultvalue"].ToLower()
            || searchText.Contains(nc.NotificationName.ToLower()) || nc.NotificationName.ToLower().Contains(searchText)
            || searchText.Contains(nc.Description.ToLower()) || nc.Description.ToLower().Contains(searchText)
            || searchText.Contains(nc.UserName.ToLower()) || nc.UserName.ToLower().Contains(searchText)) {
            return true;
        }
