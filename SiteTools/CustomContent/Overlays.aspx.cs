...
public partial class SiteTools_Overlays : System.Web.UI.Page {
    ...
    private void BuildOverlays() {
                    ...
                    if (Roles.IsUserInRole(_username, ServerSettings.AdminUserName) || coll.UserName.ToLower() == _username.ToLower()) {
                        ...
                        if (!isEditMode) {
                            ...
                            if (string.IsNullOrEmpty(hf_EditOverlay.Value)) {
                                ...
                                str.Append("<td align='center' class='border-right border-bottom'><div class='float-left' style='height: 22px;'></div></td>");
                                str.Append("<td align='center' class='border-right border-bottom'></td></tr>");
                            }
                        }
                        else {
        ...
        if (count == 0)
            str.Append("<div class='emptyGridView'>No Overlays Found</div>");
            ...
            if (!string.IsNullOrEmpty(overlayId)) {
                if (!string.IsNullOrEmpty(coll.OverlayName)) {
                    if (Roles.IsUserInRole(_username, ServerSettings.AdminUserName) || coll.UserName.ToLower() == _username.ToLower()) {
                        ...
                        str.Append("<td align='left' class='border-right border-bottom'><div class='float-left' style='height: 22px;'></div>" + coll.OverlayName + "</td>");
    ...
    protected void btn_uploadOverlay_Clicked(object sender, EventArgs e) {
        ...
        ServerSettings.PageToolViewRedirect(this.Page, "Overlays.aspx?action=" + action + "&date=" + ServerSettings.ServerDateTime.Ticks);
    }
    ...
    private bool SearchFilterValid(WorkspaceOverlay_Coll wo) {
        ...
        if (string.IsNullOrEmpty(searchText) || searchText == tb_search.Attributes["data-defaultvalue"].ToLower()
            || searchText.Contains(wo.OverlayName.ToLower()) || wo.OverlayName.ToLower().Contains(searchText)
            || searchText.Contains(wo.Description.ToLower()) || wo.Description.ToLower().Contains(searchText)
            || searchText.Contains(wo.UserName.ToLower()) || wo.UserName.ToLower().Contains(searchText)
            || (!string.IsNullOrEmpty(wo.DisplayType) && (searchText.Contains(wo.DisplayType.ToLower()) 
            || wo.DisplayType.ToLower().Contains(searchText)))) {
            return true;
        }
