...
public partial class CustomTablesAppLoader : UserControl {
    ...
    protected void Page_Load(object sender, EventArgs e) {
        ...
        if (userControl != null) {
            ...
            foreach (CustomTableCustomizations customizations in coll.TableCustomizations) {
                switch (customizations.customizeName) {
                    case "ShowDescriptionOnApp":
                        ...
                        break;
                    case "MonthSelector":
                        HiddenField hf_monthSelector = GetHiddenFieldControl("hf_" + appClassName + "_monthSelector");
                        if (hf_monthSelector != null) {
                            hf_monthSelector.Value = customizations.customizeValue;
                        }
                        break;
            ...
            ChartType chart_Type = ctv.GetChartTypeFromCustomizations(coll.TableCustomizations);
            string chartColumns = ctv.GetChartColumnsFromCustomizations(coll.TableCustomizations);
            ...
            if (chart_Type != ChartType.None && chartColumns.Split(ServerSettings.StringDelimiter_Array, StringSplitOptions.RemoveEmptyEntries).Length > 0) {
                string chartTitle = ctv.GetChartTitleFromCustomizations(coll.TableCustomizations);
            ...
            _appInitializer = new OpenWSE_Tools.Apps.AppInitializer(app_id, userName, Page, "Custom Tables");
            _appInitializer.LoadScripts_JS(true, "customTables.Load('" + appClassName + "');");
        }
    }
