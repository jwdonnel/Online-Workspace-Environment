...
public class CustomTableCreator : System.Web.Services.WebService {
    ...
    [WebMethod]
    public string Create(string columns, string summaryData, string customizations, string tableName, string description, string installForUser, string notifyUsers, string isPrivate, string usersAllowed) {
        if (_canContinue) {
            ...
            try {
                ...
                summaryData = HttpUtility.UrlDecode(summaryData);
                ...
                if (columnCreater != null && columnCreater.Length > 0) {
                    ...
                    if (CreateCustomTable(tableID, columnList)) {
                        string fileName = ctwc.CreateApp(tableName, categoryId, description, DBImporter.DefaultDatabaseIcon, true, HelperMethods.ConvertBitToBoolean(isPrivate), HelperMethods.ConvertBitToBoolean(notifyUsers));
                        CustomTableViewer ctv = new CustomTableViewer(_userName);
                        if (!string.IsNullOrEmpty(fileName)) {
                            fileName = "app-" + fileName;
                        }
                        ctv.AddItem(tableID, tableName, description, fileName, columns, summaryData, _userName, HelperMethods.ConvertBitToBoolean(notifyUsers), usersAllowed, customizations);
    ...
    [WebMethod]
    public void UploadAppIcon_Url(string file, string tableId, string tableType) {
        try {
                ...
                if (!string.IsNullOrEmpty(appId)) {
                    ...
                    if (!string.IsNullOrEmpty(appItem.ID)) {
                        ...
                        if (currentIcon.ToLower() == App.DefaultAppIconLocation.ToLower() + App.DefaultAppIcon.ToLower() || currentIcon.ToLower() == App.DefaultAppIconLocation.ToLower() + DBImporter.DefaultDatabaseIcon.ToLower()) {
                            currentIcon = App.CreateFullAppIconPath(appItem.filename, appItem.AppId.Replace("app-", string.Empty) + ".png");
                        }
                        ...
                        if (_imageConverter.SaveNewImg(file, filePath)) {
    ...
    public object[] EditTable(string id) {
        ...
        if (_canContinue) {
            try {
                ...
                obj.Add(columnsSerializer.Serialize(coll.SummaryData));
            }
            catch { }
        }
    ...
    private string GetEditableUsers(string tableId) {
        ...
        if (!string.IsNullOrEmpty(createdBy)) {
            ...
            string checkboxInput = "<div class='checkbox-click float-left pad-right-big margin-right-big pad-top pad-bottom' style='min-width: 150px;'><input type='checkbox' class='checkbox-usersallowed float-left margin-right-sml' {0} value='{1}' style='margin-top: {2};' />&nbsp;{3}</div>";
            ...
            foreach (MembershipUser membershipUser in userColl) {
                ...
                if (BasePage.IsUserNameEqualToAdmin(user)) {
                    continue;
                }
    ...
    public string RecreateApp(string appId, string tableId, string tableName, string description) {
        if (_canContinue) {
            ...
            try {
                ...
                if (!string.IsNullOrEmpty(appId)) {
                    Apps_Coll appColl = apps.GetAppInformation("app-" + appId);
                    if (!string.IsNullOrEmpty(appColl.ID)) {
                        insertIntoAppList = false;
                    }

                    ctwc.CreateApp(tableName, categoryId, description, DBImporter.DefaultDatabaseIcon, insertIntoAppList, false, appColl.AllowNotifications, appId);
                }
                else {
                    appId = ctwc.CreateApp(tableName, categoryId, description, DBImporter.DefaultDatabaseIcon, insertIntoAppList, false, false);
    ...
    [WebMethod]
    public string Update(string tableID, string columns, string summaryData, string customizations, string tableName, string description, string notifyUsers, string usersAllowed) {
        if (_canContinue) {
            ...
            if (foundTable) {
                try {
                    ...
                    if (columnCreater != null && columnCreater.Length > 0) {
                        ...
                        columnList.Add(DatabaseCall.ApplicationIdString);
                        ...
                        if (UpdateCustomTable(tableID, columnListStr, columnList, dbViewer.dt)) {
                            CustomTableViewer ctv = new CustomTableViewer(_userName);
                            ctv.UpdateRow(tableID, tableName, description, columns, summaryData, HelperMethods.ConvertBitToBoolean(notifyUsers), usersAllowed, customizations);
                            ...
                            if (!string.IsNullOrEmpty(appId)) {
                                ...
                                if (!string.IsNullOrEmpty(x.ID)) {
                                    apps.UpdateAllowNotification(x.AppId, HelperMethods.ConvertBitToBoolean(notifyUsers));
                                }
                            }
