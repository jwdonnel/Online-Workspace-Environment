...
public class AppPackages {
    ...
    public AppPackages(bool getvalues) {
        if (getvalues) {
            dataTable = dbCall.CallSelect("AppPackages", "", new List<DatabaseQuery>() { new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) }, "PackageName ASC");
        }
    }
    ...
    public void SelectPackage(string id) {
        dataTable = dbCall.CallSelect("AppPackages", "", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) }, "PackageName ASC");
    }

    public string[] GetAppList(string id) {
        if (!string.IsNullOrEmpty(id) && id != "0") {
            DatabaseQuery dbSelect = dbCall.CallSelectSingle("AppPackages", "AppList", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    ...
    public void updatePackageName(string id, string name, string user) {
        ...
        dbCall.CallUpdate("AppPackages", updateQuery, new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }
    ...
    public void deletePackage(string id) {
        dbCall.CallDelete("AppPackages", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }
}