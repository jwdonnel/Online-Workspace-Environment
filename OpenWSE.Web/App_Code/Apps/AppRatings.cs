...
public class AppRatings
{
    ...
    public List<AppRatings_Coll> GetAppRatings(string appId) {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect(_tableName, "", new List<DatabaseQuery>() { new DatabaseQuery("AppID", appId), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    ...
    public List<AppRatings_Coll> GetAllUserAppRatings(string userName) {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect(_tableName, "", new List<DatabaseQuery>() { new DatabaseQuery("UserName", userName), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
        foreach (Dictionary<string, string> dicVal in dbSelect) {
            string id = dicVal["ID"];
    ...
    public string GetAverageRating(string appId) {
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect(_tableName, "Rating", new List<DatabaseQuery>() { new DatabaseQuery("AppID", appId), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
