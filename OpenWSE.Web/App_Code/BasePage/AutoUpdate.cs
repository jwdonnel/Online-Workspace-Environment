...
public class AutoUpdate : System.Web.Services.WebService {
    ...
    public object[] StartAUSystem(string _appId) {
        ...
        if (HttpContext.Current != null && HttpContext.Current.User != null && HttpContext.Current.User.Identity != null && HttpContext.Current.User.Identity.IsAuthenticated) {
            ...
            // Delete UserUnloadSessionId if needed
            if (!string.IsNullOrEmpty(username) && !string.IsNullOrEmpty(MemberDatabase.GetUserUnloadSessionId(username))) {
                MemberDatabase.DeleteUserUnloadSessionId(username);
            }
            ...
            for (var i = 0; i < _loops; i++) {
                ...
                if (!string.IsNullOrEmpty(username) && !string.IsNullOrEmpty(_sessionId) && MemberDatabase.GetUserUnloadSessionId(username) == _sessionId) {
                    MemberDatabase.DeleteUserUnloadSessionId(username);
                    _result = "TURNOFF";
                    break;
                }
        ...
        if (AutoUpdateSystem.EnableDetailedLogging) {
            OpenWSE_Tools.Logging.BackgroundServiceLog bsl = new OpenWSE_Tools.Logging.BackgroundServiceLog();
            if (bsl.DoesLogInformation(AutoUpdateSystem.AutoUpdateServiceNamespace)) {
                string message = string.Format("{0} - {1} - {2}", username, _appId, _result);
                bsl.AddItem(AutoUpdateSystem.AutoUpdateServiceNamespace, message);
            }
        }
