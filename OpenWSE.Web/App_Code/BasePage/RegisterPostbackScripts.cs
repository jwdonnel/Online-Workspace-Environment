...
public class RegisterPostbackScripts {
    ...
    private static List<Apps_Coll> RebuildUserAppList(App _apps, MemberDatabase _member, bool AssociateWithGroups) {
        ...
        foreach (var w in memberApps) {
            ...
            if ((_member.Username != dt.CreatedBy.ToLower()) && (dt.IsPrivate)) {
                continue;
            }
    ...
    private static string LoadApp1(Page _page, App _apps, string _username) {
        ...
        HiddenField hf_loadApp1 = (HiddenField)BasePage.GetMasterPageControl(_page).FindControl("hf_loadApp1");
        ...
        if ((hf_loadApp1 != null) && (PlaceHolder1 != null)) {
            if (!string.IsNullOrEmpty(hf_loadApp1.Value)) {
                ...
                if (!string.IsNullOrEmpty(filename) && HelperMethods.IsValidAscxOrDllFile(filename)) {
                    try {
                        UserControl uc = HelperMethods.CreateUserControl(_page, "~/Apps/" + filename);
                        if (uc != null) {
    ...
    private static string LoadOverlay1(Page _page, App _apps, string _username, string _workspaceMode) {
        ...
        if (!string.IsNullOrEmpty(pnlId)) {
            HiddenField hf_loadOverlay1 = (HiddenField)BasePage.GetMasterPageControl(_page).FindControl("hf_loadOverlay1");
    ...
    private static string SearchSite(Page _page, App _apps, string _username) {
        string returnVal = string.Empty;
        HiddenField hf_SearchSite = (HiddenField)BasePage.GetMasterPageControl(_page).FindControl("hf_SearchSite");
        ...
        if (hf_SearchSite != null) {
            ...
            if (!string.IsNullOrEmpty(search)) {
                ...
                if (!string.IsNullOrEmpty(id)) {
                    ...
                    if (tempSearch.ToLower().Contains(".html") || tempSearch.ToLower().Contains(".aspx")) {
                        if (tempSearch.StartsWith("~/")) {
                            tempSearch = _page.ResolveUrl(tempSearch);
                        }
                        else if (tempSearch.ToLower().EndsWith(".aspx")) {
                            tempSearch = tempSearch.ToLower().Replace(".aspx", string.Empty);
                        }
                    }

                    tempSearch = tempSearch.ToLower();
                    foreach (string p in pages) {
                        string[] pageInfo = ServerSettings.GetAdminPageLink(p);
                        if (p.ToLower() == tempSearch || (pageInfo.Length > 0 && pageInfo[0].ToLower() == tempSearch)) {
                            if (ServerSettings.AdminPagesCheck(p, _username)) {
                                if (pageInfo.Length == 2) {
                                    returnVal = "openWSE.PopOutTool(\"" + pageInfo[0] + "\");";
                                }
                            }
                            break;
                        }
    ...
    private static string ReloadApp(Page _page, App _apps, string _username, Dictionary<string, string> _demoMemberDatabase) {
        ...
        HiddenField hf_ReloadApp = (HiddenField)BasePage.GetMasterPageControl(_page).FindControl("hf_ReloadApp");
        PlaceHolder PlaceHolder1 = (PlaceHolder)BasePage.GetMasterPageControl(_page).FindControl("PlaceHolder1");
    ...
    private static string Aboutstatsapp(Page _page, MemberDatabase _member, App _apps, string _username) {
        string returnVal = string.Empty;
        HiddenField hf_aboutstatsapp = (HiddenField)BasePage.GetMasterPageControl(_page).FindControl("hf_aboutstatsapp");
        ...
        if (hf_aboutstatsapp != null) {
            ...
            if (vals.Length == 2) {
                ...
                string appId = vals[1];
                if (!string.IsNullOrEmpty(appId)) {
                    UpdatePanel updatepnl_aboutHolder = (UpdatePanel)BasePage.GetMasterPageControl(_page).FindControl("updatepnl_aboutHolder");
                    Panel pnl_aboutHolder = (Panel)BasePage.GetMasterPageControl(_page).FindControl("pnl_aboutHolder");
                    HtmlAnchor btn_uninstallApp = (HtmlAnchor)BasePage.GetMasterPageControl(_page).FindControl("btn_uninstallApp");
            ...
            foreach (UserOverlay_Coll userOverlay in _workspaceOverlays.UserOverlays) {
                ...
                if (!string.IsNullOrEmpty(fileLoc)) {
                    if (File.Exists(ServerSettings.GetServerMapLocation + fileLoc)) {
                        UserControl uc = HelperMethods.CreateUserControl(_page, "~/" + fileLoc);
                        if (uc != null) {
                            bool _isUpdatePnl = HasUpdatePanel(uc.Controls, false);
                            if (_isUpdatePnl) {
                                RefreshOverlays _ro = new RefreshOverlays(_username, ServerSettings.GetServerMapLocation, _page, pnlId);
                    ...
                    foreach (string oId in splitOverlays) {
                        ...
                        if (!string.IsNullOrEmpty(userOverlay.FileLocation)) {
                            if (File.Exists(ServerSettings.GetServerMapLocation + userOverlay.FileLocation)) {
                                UserControl uc = HelperMethods.CreateUserControl(_page, "~/" + userOverlay.FileLocation);
                                if (uc != null) {
                                    bool _isUpdatePnl = HasUpdatePanel(uc.Controls, false);
                                    if (_isUpdatePnl) {
    ...
    private static string ReBuildPostbackApp(Apps_Coll dr, App _apps, Page _page, MemberDatabase member) {
        ...
        if (!string.IsNullOrEmpty(filename)) {
            ...
            if ((fi.Extension.ToLower() != ".exe") && (fi.Extension.ToLower() != ".com") && (fi.Extension.ToLower() != ".pif")
                ...
                AppBuilder wb = new AppBuilder((ContentPlaceHolder)BasePage.GetMasterPageControl(_page).FindControl("MainContent"), member);
    ...
    private static string ReBuildPostbackApp_NoLogin(Apps_Coll dr, App _apps, Page _page, Dictionary<string, string> _demoMemberDatabase) {
        ...
        if (!string.IsNullOrEmpty(filename)) {
            ...
            if ((fi.Extension.ToLower() != ".exe") && (fi.Extension.ToLower() != ".com") && (fi.Extension.ToLower() != ".pif")
                ...
                AppBuilder wb = new AppBuilder((ContentPlaceHolder)BasePage.GetMasterPageControl(_page).FindControl("MainContent"), _demoMemberDatabase);
