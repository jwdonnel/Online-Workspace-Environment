...
public class GroupIPListener {
    ...
    public void GetGroupIPs(string groupId) {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_GroupIPListener", "", new List<DatabaseQuery>() { new DatabaseQuery("GroupId", groupId), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) }, "IPAddress ASC");
    ...
    public bool IpIsActive(string id) {
        ...
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("aspnet_GroupIPListener", "Active", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    ...
    public bool IpIsActive(string groupId, string ipAddress) {
        ...
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("aspnet_GroupIPListener", "Active", new List<DatabaseQuery>() { new DatabaseQuery("GroupId", groupId), new DatabaseQuery("IPAddress", ipAddress), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
        string active = dbSelect.Value;
        if (HelperMethods.ConvertBitToBoolean(active)) {
            isActive = true;
        }
    ...
    public string GetIPAddress(string id) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("aspnet_GroupIPListener", "IPAddress", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }

    public string GetGroupIdFromId(string id) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("aspnet_GroupIPListener", "GroupId", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }
    ...
    public bool HasAtLeastOneActive(string GroupId) {
        ...
        Guid tempGuid = new Guid();
        if (Guid.TryParse(GroupId, out tempGuid)) {
            List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_GroupIPListener", "ID", new List<DatabaseQuery>() { new DatabaseQuery("GroupId", GroupId), new DatabaseQuery("Active", "1"), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
            if (dbSelect.Count > 0) {
                result = true;
            }
        }
    ...
    public bool HasAtLeastOneActive(string GroupId, string ipAddress) {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_GroupIPListener", "IPAddress", new List<DatabaseQuery>() { new DatabaseQuery("GroupId", GroupId), new DatabaseQuery("Active", "1"), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
        foreach (Dictionary<string, string> row in dbSelect) {
    ...
    public void DeleteRow(string id) {
        dbCall.CallDelete("aspnet_GroupIPListener", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }
    ...
    public bool CheckGroupNetwork(string groupId) {
        try {
            if (HttpContext.Current != null && HttpContext.Current.User != null && HttpContext.Current.User.Identity != null &&
                HttpContext.Current.User.Identity.Name.ToLower() == ServerSettings.AdminUserName.ToLower()) {
                    return true;
            }
