...
public class Chat
{
    ...
    public Chat(bool getvalues)
    {
        if (getvalues)
        {
            ...
            List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_Chat", "", new List<DatabaseQuery>() { new DatabaseQuery("SessionDate", ServerSettings.ServerDateTime.ToShortDateString()), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) }, "Date ASC");
    ...
    public void DeleteChatLog() {
        dbCall.CallDelete("aspnet_Chat", new List<DatabaseQuery>() { new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }

    public void DeleteUserChats(string userName) {
        dbCall.CallDelete("aspnet_Chat", new List<DatabaseQuery>() { new DatabaseQuery("tUser", userName), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
        dbCall.CallDelete("aspnet_Chat", new List<DatabaseQuery>() { new DatabaseQuery("sUser", userName), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }
    ...
    {
        List<ChatColl> temp = new List<ChatColl>();
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_Chat", "", new List<DatabaseQuery>() { new DatabaseQuery("SessionDate", dateSelected), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) }, "Date ASC");
        foreach (Dictionary<string, string> row in dbSelect) {
            string id = row["ID"];
            string tuser = row["tUser"].ToLower();
            string suser = row["sUser"].ToLower();
        ...
        for (var i = 0; i < dt.Count; i++) {
            ...
            if (((tuser == thistuser) && (suser == thissuser))
                ...
                if ((prevUser == thissuser) && (result < 1)) {
                    ...
                    sResponse.Append("<td valign='top' style='width: 60px;'>" + UserImageColorCreator.CreateImgColor(member.AccountImage, member.UserColor, member.UserId, 40, member.SiteTheme) + "</td>");
    ...
    {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("aspnet_Chat", "tUser", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }

    public string GetsUser(string id)
    {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("aspnet_Chat", "sUser", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }
    ...
    public DateTime GetDate(string id)
    {
        ...
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("aspnet_Chat", "Date", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    ...
    private string BuildUserImagePart(MemberDatabase member, bool alignImgLeft) {
        string userImageColor = UserImageColorCreator.CreateImgColor(member.AccountImage, member.UserColor, member.UserId, 30, member.SiteTheme);
        return string.Format("<td valign='top' align='center'>{0}</td>", userImageColor);
    }
    private string BuildMessagePart(string un, DateTime date, string id, string smsg, bool usesImg, bool flipDateName) {
        ...
        if (usesImg) {
            ...
            if (un.ToLower() == "you") {
                sResponse.Append("<td valign='top'><div class='pad-left'>");
            }
            else {
                sResponse.Append("<td valign='top'><div class='pad-right'>");
            }
        }

        sResponse.Append("<h3>" + un + "</h3>");
        sResponse.Append("<span class='date-chat-line'>" + date.ToShortTimeString() + ServerSettings.StringDelimiter + TimeZone.CurrentTimeZone.GetUtcOffset(date).Hours + "</span>");
