...
public class DefaultDBTables {
    ...
    private static void UpdateTableColumns(DataTable dbTable, XmlNodeList columns, DatabaseCall dbCall, string tableName) {
        ...
        foreach (XmlNode xColumn in columns) {
            ...
            if (!foundColumn) {
                ...
                if (columnName == DatabaseCall.ApplicationIdString) {
                    UpdateApplicationIdRows(tableName, dbCall);
                }
    ...
    private static void UpdateApplicationIdRows(string tableName, DatabaseCall dbCall) {
        ...
        foreach (Dictionary<string, string> keyVal in tableRowList) {
            if (string.IsNullOrEmpty(keyVal[DatabaseCall.ApplicationIdString])) {
                List<DatabaseQuery> query = new List<DatabaseQuery>();
                query.Add(new DatabaseQuery(DatabaseCall.ApplicationIdString, "NULL"));

                List<DatabaseQuery> updateQuery = new List<DatabaseQuery>();
                updateQuery.Add(new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID));
                ...
                if (!dbCall.CallUpdate(tableName, updateQuery, query)) {
                    ...
                    foreach (KeyValuePair<string, string> keyVal2 in keyVal) {
                        if (keyVal2.Key != DatabaseCall.ApplicationIdString && !string.IsNullOrEmpty(keyVal2.Value)) {
                                ...
                                for (int i = 0; i < rowNode.Attributes.Count; i++) {
                                    ...
                                    if (tableNode.Attributes["key"] != null) {
                                        if (rowNode.Attributes[i].Name == tableNode.Attributes["key"].Value) {
                                            if (tableNode.Attributes["key"].Value == DatabaseCall.ApplicationIdString && string.IsNullOrEmpty(rowNode.Attributes[i].Value)) {
                                                queryFind.Add(new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID));
                                            }
                                    ...
                                    if (rowNode.Attributes[i].Name == DatabaseCall.ApplicationIdString && string.IsNullOrEmpty(rowNode.Attributes[i].Value)) {
