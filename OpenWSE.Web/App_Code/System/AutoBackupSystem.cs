...
namespace OpenWSE_Tools.AppServices {
    ...
    public class AutoBackupSystem : IBackgroundServiceState {
        ...
        public AutoBackupSystem_Coll GetEntry(string _id) {
            ...
            List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("AutoBackupSystem", "", new List<DatabaseQuery>() { new DatabaseQuery("ID", _id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
            foreach (Dictionary<string, string> row in dbSelect) {
                string id = row["ID"];
                string day = row["BackupDay"];
                string time = row["BackupTime"];
        ...
        public void deleteItem(string id) {
            dbCall.CallDelete("AutoBackupSystem", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
        ...
        public void StartService() {
            ...
            BackgroundServiceCalls.CreateServiceThread(startThread, AutoBackupSystemNamespace).Start();
        }
        ...
        public static void WriteToXml(string backupfile, string path, string desc) {
            try {
                if (File.Exists(backupfile)) {
                    ...
                    str.Append("<ApplicationID>" + ServerSettings.ApplicationID + "</ApplicationID>");
