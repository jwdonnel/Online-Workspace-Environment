...
public class ShowUpdatePopup {
    ...
    private void BuildList() {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_ShowUpdates", "", query);
    ...
    public bool isUserShowPopup(string username) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("aspnet_ShowUpdates", "ShowPopup", new List<DatabaseQuery>() { new DatabaseQuery("UserName", username.ToLower()), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    ...
    public string GetNewUpdateMessage(string serverPath, string theme, bool showLink = true) {
        ...
        foreach (KeyValuePair<string, string> entry in aboutItems) {
            ...
            string img = "<img alt='' class='float-left pad-right' src='" + ServerSettings.ResolveUrl("~/App_Themes/" + theme + "/App/approve.png") + "' style='height: 17px;' />";
