...
public class UserAppSettings {
    ...
    public void AddNewAppSetting(string appId, bool allowResize, bool allowMax, string cssClass,  bool autoFullScreen, string minHeight, string minWidth, bool allowPopOut, string popOutLoc, bool autoOpen, string defaultWorkspace, string categories, string appBackgroundColor, string iconBackgroundColor) {
            if (!string.IsNullOrEmpty(_username)) {
                ...
                if (!isUpdateNeeded) {
                    ...
                    query2.Add(new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID));
    ...
    public void DeleteAllAppSetting() {
        if (!string.IsNullOrEmpty(_username)) {
            dbCall.CallDelete(UserSettingsTableName, new List<DatabaseQuery>() { new DatabaseQuery("Username", _username), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
        }
    }
    ...
    public void BuildUserAppSettings() {
        if (!string.IsNullOrEmpty(_username) && _allowOverrides) {
            _userAppSettingList = dbCall.CallSelect(UserSettingsTableName, "", new List<DatabaseQuery>() { new DatabaseQuery("Username", _username), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
        }
    }
    ...
    public Dictionary<string, string> OverrideValues(Dictionary<string, string> defaultAppSettings) {
        ...
        foreach (Dictionary<string, string> settings in _userAppSettingList) {
            ...
            if (!defaultAppSettings.ContainsKey("AllowUserOverrides") || string.IsNullOrEmpty(defaultAppSettings["AllowUserOverrides"])) {
                allowUserOverrides = true;
            }
            else {
                allowUserOverrides = HelperMethods.ConvertBitToBoolean(defaultAppSettings["AllowUserOverrides"]);
            }

            if (defaultAppSettings.ContainsKey("AppID") && settings["AppID"] == defaultAppSettings["AppID"] && allowUserOverrides) {
