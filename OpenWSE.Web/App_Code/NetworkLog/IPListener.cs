...
public class IPListener {
    ...
    public bool TableEmpty {
        get {
            bool empty = true;
            List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_IPListener", "", new List<DatabaseQuery>() { new DatabaseQuery("Active", "1"), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
            if (dbSelect.Count > 0) {
                empty = false;
            }
            return empty;
    ...
    public bool CheckIfActive(string ip) {
        bool act = false;
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_IPListener", "", new List<DatabaseQuery>() { new DatabaseQuery("IPAddress", ip), new DatabaseQuery("Active", "1"), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
        if (dbSelect.Count > 0) {
            act = true;
        }
        return act;
    ...
    public bool CheckIfExists(string ip) {
        bool ex = false;
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_IPListener", "", new List<DatabaseQuery>() { new DatabaseQuery("IPAddress", ip), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
        if (dbSelect.Count > 0) {
            ex = true;
        }
        return ex;
    ...
    public void deleteIP(string ip) {
        dbCall.CallDelete("aspnet_IPListener", new List<DatabaseQuery>() { new DatabaseQuery("IPAddress", ip), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }
}