...
public class TwitterFeeds
{
    ...
    public TwitterFeeds(string username, bool getvalues)
    {
        if (getvalues)
        {
            dataTable = dbCall.CallSelect("TwitterFeeds", "", new List<DatabaseQuery>() { new DatabaseQuery("UserName", username), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) }, "DATE ASC");
        }
    }

    public Dictionary<string, string> GetRow(string id)
    {
        List<Dictionary<string, string>> row = dbCall.CallSelect("TwitterFeeds", "", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) }, "DATE ASC");
        if (row.Count > 0) {
            return row[0];
        }
    ...
    {
        dbCall.CallDelete("TwitterFeeds", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }

    public void deleteUserFeeds(string userName) {
        dbCall.CallDelete("TwitterFeeds", new List<DatabaseQuery>() { new DatabaseQuery("UserName", userName), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }
}