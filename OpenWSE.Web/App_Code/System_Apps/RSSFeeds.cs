...
public class RSSItem {
    ...
    public RSSItem(string title, string link, string summary, string content, string pubDate, string creator, string bgStyleClass) {
        ...
        _bgColor = HelperMethods.GetRandomColor();
    }
...
public class RSSFeeds {
    ...
    private const string SavedFeedsTableName = "aspnet_SavedRSSFeeds";
    ...
    public void BuildEntriesAll() {
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("RSSFeeds", "", new List<DatabaseQuery>() { new DatabaseQuery("UserName", _userName), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) }, "RSSFeedTitle ASC");
    ...
    public void DeleteRowByID(string id) {
        dbCall.CallDelete("RSSFeeds", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }

    public void DeleteUserFeeds(string userName) {
        dbCall.CallDelete("RSSFeeds", new List<DatabaseQuery>() { new DatabaseQuery("UserName", userName), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }

    public void DeleteRowByRSSID(string rssid) {
        dbCall.CallDelete("RSSFeeds", new List<DatabaseQuery>() { new DatabaseQuery("RSSID", rssid), new DatabaseQuery("UserName", _userName), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }

    public void DeleteRowByURL(string loc) {
        dbCall.CallDelete("RSSFeeds", new List<DatabaseQuery>() { new DatabaseQuery("RSSFeedLoc", loc), new DatabaseQuery("UserName", _userName), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }
    ...
    public void AddSavedFeed(string data) {
        List<DatabaseQuery> query = new List<DatabaseQuery>();
        query.Add(new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID));
        query.Add(new DatabaseQuery("ID", Guid.NewGuid().ToString()));
        query.Add(new DatabaseQuery("UserName", _userName));
        query.Add(new DatabaseQuery("FeedData", data));

        dbCall.CallInsert(SavedFeedsTableName, query);
    }

    public List<Dictionary<string, string>> GetSavedFeeds() {
        return dbCall.CallSelect(SavedFeedsTableName, "", new List<DatabaseQuery>() { new DatabaseQuery("UserName", _userName), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }

    public void DeleteSavedFeed(string id) {
        dbCall.CallDelete(SavedFeedsTableName, new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }
    ...
    public static string GetBackgroundImageFromFeed(string url, string summary, string content) {
        ...
        bg = HelperMethods.RemoveProtocolFromUrl(bg);
            ...
            if (isNewFeed) {
                ...
                for (int i = 0; i < rssItems.Count; i++) {
                    ...
                    if (!string.IsNullOrEmpty(rssItem.Title)) {
                        ...
                        RSSFeedUpdater.SendAlertMessages(rssItem);
                    }
                }
            }
            else {
                        ...
                        foreach (SyndicationElementExtension extension in item.ElementExtensions) {
                            ...
                            if (element.HasAttributes) {
                                foreach (var attribute in element.Attributes()) {
                                    ...
                                    if ((value.StartsWith("//") || value.StartsWith("http://") || value.StartsWith("https://") || value.StartsWith("www.")) && (value.EndsWith(".jpg") || value.EndsWith(".png") || value.EndsWith(".gif"))) {
                                        bgStyleClass = "style=\"background-image: url('" + HelperMethods.RemoveProtocolFromUrl(value) + "'); background-repeat: no-repeat; background-position: center center; background-size: cover;\" class=\"feed-preview rss-has-image\"";
                                        foundImage = true;
                                        break;
                                    }
                                }
            ...
            if (!string.IsNullOrEmpty(FeedListFile) && didNotFind && File.Exists(ServerSettings.GetServerMapLocation + FeedListFile)) {
                try {
                    ...
                    using (var str = new BinaryReader(a)) {
                        ...
                        object x = bf.Deserialize(str.BaseStream);
                        RSSFeeds.LoadedFeedList = (Dictionary<string, KeyValuePair<DateTime, IList<RSSItem>>>)x;
                        a.Close();
                        str.Close();
                    }
                }
