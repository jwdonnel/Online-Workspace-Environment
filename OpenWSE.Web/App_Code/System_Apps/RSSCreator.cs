...
public class RSSCreator {
    ...
    public void Create(string groupName) {
        ...
        using (FileStream fs = File.Create(_serverLoc + "Apps\\MessageBoard\\RSS_Feeds\\" + groupName + ".rss")) {
            ...
            xtwFeed.WriteElementString("copyright", "Copyright " + ServerSettings.ServerDateTime.Year.ToString() + " " + ServerSettings.SiteName + ". All rights reserved.");

            // Loop through the content of the database and add them to the RSS feed
            int count = 0;
            List<Dictionary<string, string>> dt = messageBoard.GetEntriesByGroup(groupName);
