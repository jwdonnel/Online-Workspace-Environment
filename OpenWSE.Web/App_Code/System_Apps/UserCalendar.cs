...
public class UserCalendar
{
    ...
    {
        dataTable = dbCall.CallSelect("aspnet_UserCalendar", "", new List<DatabaseQuery>() { new DatabaseQuery("UserName", _username.ToLower()), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) }, "StartDate ASC");
    }

    public void GetEvent(string id)
    {
        dataTable = dbCall.CallSelect("aspnet_UserCalendar", "", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }
    ...
    {
        dbCall.CallDelete("aspnet_UserCalendar", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }

    public void DeleteEvents(string userName) {
        dbCall.CallDelete("aspnet_UserCalendar", new List<DatabaseQuery>() { new DatabaseQuery("UserName", userName), new DatabaseQuery(DatabaseCall.ApplicationIdString, ServerSettings.ApplicationID) });
    }
