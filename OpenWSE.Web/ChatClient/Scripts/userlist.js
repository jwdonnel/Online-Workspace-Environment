...
var chatClient = function () {
    ...
    $(document.body).on("click", "#main_container, #container-logo, .workspace-holder", function () {
    ...
    $(document.body).on("click", ".ChatUserNotSelected, .ChatUserSelected", function () {
        ...
        if ($(".app-remote-container").length === 0) {
            ...
            window.location.hash = "#?tab=Chat_tab&id=" + user + "&fullname=" + fullName;
        }
    ...
    function BuildChatButtonHolder(id, user) {
        ...
        modal += "<div class='app-popup-inner-app-header-text'>App Options</div><ul>";
        ...
        modal += "<li onclick='openWSE.AboutApp(this)' title='About App'><div class='about-app'></div><span>About</span></li></ul><div class='clear'></div>";

        if ($("#ddl_WorkspaceSelector").length > 0) {
            modal += "<div class='pad-all'><span class='workspace-selector-app-option'>Workspace</span>";
    ...
    function BuildChatWindowMobile(user) {
        ...
        $("#pnlContent_ChatPopup").append("<iframe class='iFrame-chat' src='" + GetSiteRoot() + "ChatClient/ChatWindow.html?user=" + user + "' width='100%' frameborder='0'></iframe>");
        updateisNewChat(user);
        $(window).resize();
    }
    ...
    function updateChatUsers() {
        if (!_isUpdateUsersCall) {
            ...
            AjaxCall(pageUrl + "/CallUserList", '{ "currstatus": "' + $("#span_currStatus").html() + '","isPostBack": "' + isPostBack + '" }', {
                cache: false
            }, function (data) {
                if (data.d[0] != null)
    ...
    function updateUserList(data) {
        ...
        if ((data != "") && (data.toLowerCase() != chatuserlist.toLowerCase())) {
            ...
            var totalusers = $('.listofusers li').length;
    ...
    function updateisNewChat(userto) {
        if (!_isUpdateNewCall) {
            ...
            AjaxCall(pageUrl + "/CallUpdate", '{ "userto": "' + userto + '" }', null, function (data) {
    ...
    function updateCurrStatus(status) {
        if (!_isUpdateStatusCall) {
            ...
            AjaxCall(pageUrl + "/CallUpdateStatus", '{ "status": "' + status + '" }', null, function (data) {
                var response = data.d;
                response = response.replace(/"/gi, "'");
                var elem = document.getElementById("currentStatus");
                if ((elem.innerHTML.replace(/"/gi, "'") != response) && (response != ""))
    ...
    function updateWindowFocus() {
        if (!_isUpdateFocusCall) {
            ...
            AjaxCall(pageUrl + "/CallWindowFocus", '{ }', null, function (data) {
                var response = data.d;
                response = response.replace(/"/gi, "'");
                var elem = document.getElementById("currentStatus");
                if ((elem.innerHTML.replace(/"/gi, "'") != response) && (response != ""))
    ...
    function updateChatUsers_NoLoop() {
        AjaxCall(pageUrl + "/CallUserList", '{ "currstatus": "' + $("#span_currStatus").html() + '","isPostBack": "1" }', {
            cache: false
        }, function (data) {
            if (data.d[0] != null)
                getCurrStatus(data.d[0]);
    ...
    function AjaxCall(_url, _data, _options, successFuntion, errorFunction, completeFunction) {
        ...
        if (_options.async !== null && _options.async !== undefined) {
            ajaxObj["async"] = _options.async;
        }
