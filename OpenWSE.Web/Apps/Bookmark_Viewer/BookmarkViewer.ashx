...
public class BookmarkViewer : IHttpHandler {
    ...
    private const string noBookmarks = "<h3 class='no-bookmarks pad-right-big pad-top-big'>No bookmarks available</h3>";
    ...
    public string BuildLink(Dictionary<string, string> dr) {
        ...
        string favicon = "Apps/BookmarkViewer/unknown.png";
        string width = "25px";
        try {
            ...
            if ((uri.Host.ToLower().Contains("youtube.com")) && (uri.LocalPath == "/watch") && (uri.IsAbsoluteUri)) {
                ...
                favicon = "//img.youtube.com/vi/" + queryString + "/default.jpg";
                width = "65px";
            }
            else if (uri.IsAbsoluteUri)
                favicon = "//www.google.com/s2/favicons?domain=" + uri.OriginalString;
        }
        catch { }
        ...
        if ((html.Contains("//www.youtube.com/watch")) || (html.Contains("www.youtube.com/watch")) || (html.Contains("youtube.com/watch"))) {
            youtubeClass = " bookmark-table-YoutubeLink";
            favicon = "<img alt='favicon' src='" + favicon + "' title='Play YouTube Video' class='pad-right' onclick='bookmarkApp.embedded_video_bookmark(\"" + ConvertToObjectEmbeded(html) + "\", \"" + id + "\");' style='width:" + width + "' />";
            view = "<a href='#' class='bookmark-html-link' title='Play YouTube Video' onclick='bookmarkApp.embedded_video_bookmark(\"" + ConvertToObjectEmbeded(html) + "\", \"" + id + "\");return false;'>" + name + "</a>";
        }
        else {
        ...
        string remove = "<a href='#Remove' class='td-delete-btn margin-right-sml margin-left-sml' onclick='bookmarkApp.remove_in_iFrame(\"" + id + "\");return false;' title='Delete'></a>";
        string share = "<a href='#Share' class='td-add-btn margin-right-sml margin-left-sml' onclick='bookmarkApp.share_in_iFrame(\"" + id + "\");return false;' title='Share'></a>";
        string edit = "<a href='#Edit' class='td-edit-btn margin-right-sml margin-left-sml' onclick='bookmarkApp.edit_in_iFrame(\"" + id + "\");return false;' title='Edit'></a>";
    ...
    private string Share(string id) {
        ...
        foreach (System.Web.Security.MembershipUser u in System.Web.Security.Membership.GetAllUsers()) {
            if ((u.UserName.ToLower() != ServerSettings.AdminUserName.ToLower()) && (u.UserName.ToLower() != _member.Username)) {
        ...
        if (count == 0) {
            ...
            str.Append("<input type='button' class='input-buttons' value='Share' onclick='bookmarkApp.share_click(\"" + id + "\")' />");
        }
    ...
    private string Edit(string id, string sortBystr) {
        ...
        foreach (Dictionary<string, string> dr in _bookmarks.bookmarks_dt) {
            if (id == dr["ID"].ToString()) {
                ...
                str.Append("<div class='modal-descDivs-item'>Url</div>");
                str.Append("<input type='text' id='editBookmarkHtml' class='textEntry margin-right-sml' value=\"" + dr["BookmarkHTML"].ToString() + "\" style='width: 100%;' />");
                str.Append("<div class='clear-space-five'></div>");
                str.Append("<div class='modal-descDivs-item'>Name</div>");
                str.Append("<input type='text' id='editBookmarkName' class='textEntry margin-right-sml' value=\"" + dr["BookmarkName"].ToString() + "\" style='width: 100%;' />");
                str.Append("<div class='clear-space'></div>");
                break;
            }
        }
