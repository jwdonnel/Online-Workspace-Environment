...
public class PersonalCalendar : System.Web.Services.WebService {
        ...
        if (userID.IsAuthenticated) {
            try {
                ...
                foreach (Dictionary<string, string> dr in ce.calendar_dt) {
                    ...
                    if (DateTime.TryParse(dr["StartDate"], out startmonth)) {
                        ...
                        if (DateTime.TryParse(dr["EndDate"], out endmonth)) {
                            ...
                            if (InBetweenDaysInclusiveForOverview(ServerSettings.ServerDateTime.Date, startmonth, endmonth)) {
                ...
                if (!string.IsNullOrEmpty(str.ToString()))
                    return str.ToString() + "<div class='clear'></div>";
            }
            catch { }
        }
        
        return "<h4>No events available</h4><div class='clear-space'></div>";
    }
        ...
        if (userID.IsAuthenticated) {
            try {
                ...
                if (ce.calendar_dt.Count == 1) {
                    ...
                    string usercolor = "<div class='sch_ColorCode float-left rounded-corners-15' style='border:1px solid #DDD;width:10px;height:10px;margin-top:8px;color:transparent;background-color:" + dr["ColorCode"] + "'></div>";
                    str.Append("<div class='float-left pad-right'><img alt='' src='Apps/PersonalCalendar/Images/plan.png' /></div>");
                    str.Append("<div class='float-left'><h2><b>" + usercolor + dr["Title"] + "</b></h2><div class='clear-space-two'></div>");
                    str.Append("<small><b class='pad-right'>Updated</b>" + dr["DateUpdated"] + "</small></div>");
                    str.Append("<div class='clear'></div>");
                    str.Append("<div class='clear-space'></div>");
                    str.Append("<b class='modal-descDivs-item'>Start Time</b><span>" + dr["StartDate"] + "</span><div class='clear-space-five'></div>");
                    str.Append("<b class='modal-descDivs-item'>End Time</b><span>" + dr["EndDate"] + "</span><div class='clear-space-five'></div>");
                    str.Append("<b class='modal-descDivs-item'>Description</b>");
                    string desc = dr["Description"];
                    
                    if (string.IsNullOrEmpty(desc))
                        desc = "No description available";

                    str.Append("<span>" + desc + "</span><div class='clear'></div>");
                    returnVals.Add(str.ToString());

                    str = new StringBuilder();
                    str.Append("<input type='button' class='input-buttons modal-cancel-btn' value='Close' onclick=\"personalCalendarApp.ViewNewCalendarEvent();\" />");
                    str.Append("<input type='button' class='input-buttons modal-ok-btn' value='Delete' onclick=\"personalCalendarApp.DeleteEventInfo('" + dr["ID"] + "');\" />");

                    string editClick = " onclick=\"personalCalendarApp.EditCalendarEvent('" + dr["Title"] + "','" + dr["Description"] + "','" + dr["ColorCode"] + "','" + dr["StartDate"] + "','" + dr["EndDate"] + "');\"";
                    str.Append("<input type='button' class='input-buttons modal-ok-btn' value='Edit' " + editClick + " />");
                    returnVals.Add(str.ToString());
    ...
    public object[] GetMonthInfo(string month, string year, string search) {
        ...
        object[] returnObj2 = new object[2];
        returnObj2[0] = returnObj.ToArray();
        returnObj2[1] = GetOverview();
        return returnObj2;
    }
    ...
    public static bool InBetweenDaysInclusiveForOverview(DateTime datetime, DateTime start, DateTime end) {
        if ((datetime.Ticks >= start.Ticks && datetime.Ticks <= end.Ticks) || (datetime.Month == start.Month && datetime.Year == start.Year && start.Day >= datetime.Day))
            return true;
