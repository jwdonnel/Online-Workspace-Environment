...
public partial class Apps_iFrames_DeliveryPickups : Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (_appInitializer.TryLoadPageEvent)
        {
            ...
            if (!IsPostBack)
            {
                ...
                hf_dateselected_deliverypickups.Value = ServerSettings.ServerDateTime.Month + "/" + ServerSettings.ServerDateTime.Day + "/" +
                                                        ServerSettings.ServerDateTime.Year;
                Calendar1_deliverypickups.SelectedDate = new DateTime(ServerSettings.ServerDateTime.Year, ServerSettings.ServerDateTime.Month,
                                                                      ServerSettings.ServerDateTime.Day);
                lbl_currDate_deliverypickups.Text = ServerSettings.ServerDateTime.Month + "/" + ServerSettings.ServerDateTime.Day + "/" +
                                                    ServerSettings.ServerDateTime.Year;
                string x = ServerSettings.ServerDateTime.ToLongDateString();
    ...
    private void getNextApp()
    {
        ...
        DateTime currTime = ServerSettings.ServerDateTime;
        ...
        while (hourLong <= 22)
        {
                ...
                (Convert.ToDateTime(date).DayOfWeek != DayOfWeek.Sunday))
            {
                ...
                else
                {
                    ...
                    TimeSpan newTime = ServerSettings.ServerDateTime.Subtract(Convert.ToDateTime(date));
    ...
    private void SearchTable()
    {
        ...
        hf_dateselected_deliverypickups.Value = ServerSettings.ServerDateTime.Month - 1 + "/" + ServerSettings.ServerDateTime.Day + "/" +
                                                ServerSettings.ServerDateTime.Year;
        ...
        else
        {
            ...
            else
            {
                _schedule.addItem(schedule_name_deliverypickups.Value, schedule_trucknum_deliverypickups.Value,
                                 schedule_from_deliverypickups.Value, schedule_items_deliverypickups.Value, phonenum,
                                 schedule_email_deliverypickups.Value, getSchType(), ServerSettings.ServerDateTime.ToString(), date,
                                 schedule_comment_deliverypickups.Value, cn);
                displaySuccess(date2, cn);
            }
        }
                               ...
                               string email, string type, string schdate, string comment, string cn)
    {
        try
        {
            try
            {
                ...
                _schedule.addItem(company, trucknum, Delfrom, items, phonenumber, email, type, ServerSettings.ServerDateTime.ToString(CultureInfo.InvariantCulture),
                                 schdate, comment, cn);
            }
            catch (Exception ex) {
                AppLog.AddError(ex);
    ...
    private void buildTimeSlot(int m, int d, int y)
    {
        ...
        if ((((m == ServerSettings.ServerDateTime.Month) && (d >= ServerSettings.ServerDateTime.Day) && (y >= ServerSettings.ServerDateTime.Year))
             || ((m > ServerSettings.ServerDateTime.Month) && (y >= ServerSettings.ServerDateTime.Year)))
            && ((checkWeekends.DayOfWeek != DayOfWeek.Saturday) && (checkWeekends.DayOfWeek != DayOfWeek.Sunday)))
        {
            ...
            while (hl <= 22)
            {
                ...
                if (checkTimeSlot(date))
                {
                    if ((m == ServerSettings.ServerDateTime.Month) && (d == ServerSettings.ServerDateTime.Day) && (y == ServerSettings.ServerDateTime.Year))
                    {
                        if ((hl == ServerSettings.ServerDateTime.Hour) && (minLong > ServerSettings.ServerDateTime.Minute))
                        {
                        ...
                        {
                            if (hl > ServerSettings.ServerDateTime.Hour)
                            {
    ...
    private bool checkTimeSlot(string date)
    {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("SELECT * FROM Scheduler WHERE ScheduleDate='" + date + "'");
