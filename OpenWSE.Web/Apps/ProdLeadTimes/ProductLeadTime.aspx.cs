...
public partial class Apps_ProdLeadTimes_ProductLeadTime : System.Web.UI.Page {
    ...
    protected void tb_Date6_TextChanged(object sender, EventArgs e) {
        checkforerrors(tb_Date6_ProductLeadTime);
    }
    ...
    protected void CheckBox6_CheckedChanged(object sender, EventArgs e) {
        if (CheckBox6_ProductLeadTime.Checked) {
            tb_Date6_ProductLeadTime.Text = "Out of Service";
            tb_Date6_ProductLeadTime.Enabled = false;
        }
        else {
            tb_Date6_ProductLeadTime.Text = string.Empty;
            tb_Date6_ProductLeadTime.Enabled = true;
        }
        LoadPLTHistory(ref GV_PLTHistory_ProductLeadTime, "0", "desc");
    }

    protected void CheckBox6_6CheckedChanged(object sender, EventArgs e) {
        if (CheckBox6_6_ProductLeadTime.Checked) {
            _plt.UpdateOverride6(true);
            CheckBox6_ProductLeadTime.Enabled = true;
            CheckBox6_ProductLeadTime.Visible = true;
        }
        else {
            _plt.UpdateOverride6(false);
            CheckBox6_ProductLeadTime.Enabled = false;
            CheckBox6_ProductLeadTime.Visible = false;
        }
        getPLT();
        LoadPLTHistory(ref GV_PLTHistory_ProductLeadTime, "0", "desc");
    }
    ...
    protected void btn_update_Click(object sender, EventArgs e) {
        if ((string.IsNullOrEmpty(tb_Date1_ProductLeadTime.Text)) || (string.IsNullOrEmpty(tb_Date2_ProductLeadTime.Text))
            ...
            try {
                string DateTimeNow = ServerSettings.ServerDateTime.ToString(CultureInfo.InvariantCulture);
                _plt.addItem(tb_Date1_ProductLeadTime.Text, tb_Date2_ProductLeadTime.Text, tb_Date3_ProductLeadTime.Text, tb_Date4_ProductLeadTime.Text, tb_Date5_ProductLeadTime.Text, tb_Date6_ProductLeadTime.Text, DateTimeNow, HttpContext.Current.User.Identity.Name);
                if (_plt.pltcoll.Count > 0) {
                    lbl_lastupdated_ProductLeadTime.Text = DateTimeNow;
                    var tempmember = new MemberDatabase(HttpContext.Current.User.Identity.Name);
                    lbl_updatedby_ProductLeadTime.Text = HelperMethods.MergeFMLNames(tempmember);
                }
                AddNotifications(tb_Date1_ProductLeadTime.Text, tb_Date2_ProductLeadTime.Text, tb_Date3_ProductLeadTime.Text, tb_Date4_ProductLeadTime.Text, tb_Date5_ProductLeadTime.Text, tb_Date6_ProductLeadTime.Text, DateTimeNow, HttpContext.Current.User.Identity.Name);
                _plt = new plt(true);
                getPLT();
                LoadPLTHistory(ref GV_PLTHistory_ProductLeadTime, "0", "desc");
            }
    ...
    private void AddNotifications(string date1, string date2, string date3, string date4, string date5, string date6, string updated, string by) {
        ...
        messagebody += "<b><u>CTL 2 (Big)</u></b></td><td><b><u>1/4'' Shear</u></b></td><td><b><u>1/2'' Shear</u></b></td><td><b><u>Laser Cutter</u></b></td>";
        messagebody += "<td><b><u>Burn Table</u></b></td></tr><tr><td>" + date1 + "</td><td>" + date2 + "</td><td>" + date3 + "</td>";
        messagebody += "<td>" + date4 + "</td><td>" + date6 + "</td><td>" + date5 + "</td></tr></tbody></table>";
    ...
    public DataView GetList() {
        ...
        dtlist.Columns.Add(new System.Data.DataColumn("lasertable"));
        ...
        foreach (plt_coll row in _plt.pltcoll) {
            ...
            drlist["lasertable"] = row.LaserTable;
