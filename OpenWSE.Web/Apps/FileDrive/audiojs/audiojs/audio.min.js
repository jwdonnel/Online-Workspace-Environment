(function (h, o, g) {
    ...
    g[h] = {
        ...
        settings: {
            ...
            css: '        .audiojs audio { position: absolute; left: -1px; }         .audiojs { width: 460px; height: 36px; overflow: hidden; font-family: "Segoe UI",Arial,Sans-Serif; }         .audiojs .play-pause { display: none; width: 25px; height: 40px; padding: 4px 6px; margin: 0px; float: left; overflow: hidden; border-right: 1px solid #000; }         .audiojs p { display: none; width: 25px; height: 40px; margin: 0px; cursor: pointer; }         .audiojs .play { display: block; }         .audiojs .scrubber { position: relative; float: left; width: 280px; height: 7px; margin: 16px 10px 10px 0; border-left: 0px; border-bottom: 0px; overflow: hidden; }         .audiojs .progress { position: absolute; top: 0px; left: 0px; height: 7px; width: 0px; background: #F5A740; z-index: 1; cursor: pointer; }         .audiojs .loaded { position: absolute; top: 0px; left: 0px; height: 7px; width: 0px; background: #CCC; cursor: pointer; }         .audiojs .time { float: left; height: 36px; line-height: 36px; margin: 0px 0px 0px 6px; padding: 0px 6px 0px 12px; border-left: 1px solid #000; color: #555; }         .audiojs .time em { padding: 0px 2px 0px 0px; color: #555; font-style: normal; font-size: 13px; }         .audiojs .time strong { padding: 0px 0px 0px 2px; font-weight: normal; color: #555; font-size: 13px; }         .audiojs .error-message { float: left; display: none; margin: 0px 10px; height: 36px; width: 400px; overflow: hidden; line-height: 36px; white-space: nowrap; color: #fff;           text-overflow: ellipsis; -o-text-overflow: ellipsis; -icab-text-overflow: ellipsis; -khtml-text-overflow: ellipsis; -moz-text-overflow: ellipsis; -webkit-text-overflow: ellipsis; }         .audiojs .error-message a { color: #eee; text-decoration: none; padding-bottom: 1px; border-bottom: 1px solid #999; white-space: wrap; }                 .audiojs .play { background: url("$1") -2px -1px no-repeat; }         .audiojs .loading { background: url("$1") -2px -31px no-repeat; }         .audiojs .error { background: url("$1") -2px -61px no-repeat; }         .audiojs .pause { background: url("$1") -2px -91px no-repeat; }                 .playing .play, .playing .loading, .playing .error { display: none; }         .playing .pause { display: block; }                 .loading .play, .loading .pause, .loading .error { display: none; }         .loading .loading { display: block; }                 .error .time, .error .play, .error .pause, .error .scrubber, .error .loading { display: none; }         .error .error { display: block; }         .error .play-pause p { cursor: auto; }         .error .error-message { display: block; }',
            trackEnded: function () { },
            flashError: function () {
    ...
    g[o].prototype = {
        ...
        trackEnded: function () {
            ...
            try {
                ...
                $document.find(".equalizer-img").remove();
                $document.find(".equalizer-holder").hide();
                $document.find(".audio").attr("src", $document.find(".audio").attr("src").replace("stop.png", "play.png"));
                $document.find("#audioPlayer").css("min-height", "0px");

                if ($document.find("#continue_play_hidden").val() != "false" || $document.find("#repeat_play_hidden").val() == "true") {
                    var foundCurrent = false;
                    var nextIndex = 0;
                    for (var i = 0; i < $document.find("#pnl_FilesDocuments").find(".audio-file").length; i++) {
                        if (foundCurrent) {
                            nextIndex = i;
                            break;
                        }
                        else if ($document.find("#pnl_FilesDocuments").find(".audio-file").eq(i).attr("data-src") == this.mp3) {
                            foundCurrent = true;
                            if ($document.find("#repeat_play_hidden").val() == "true") {
                                nextIndex = i;
                                break;
                            }
                        }
                    ...
                    if ($document.find("#repeat_play_hidden").val() == "false" && $document.find("#shuffle_play_hidden").val() == "true") {
                        nextIndex = Math.floor((Math.random() * $("#pnl_FilesDocuments").find(".audio-file").length));
                    }

                    $document.find(".audio").eq(nextIndex).trigger("click");
                }
                else {
                    $document.find("#audioPlayer").html("");
                }
            }
            catch (evt) { }
        }
