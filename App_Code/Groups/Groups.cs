...
namespace OpenWSE_Tools.GroupOrganizer {
    ...
    public class Groups {
        ...
        public void getEntries(string groupId) {
            ...
            dataTable = dbCall.CallSelect("GroupList", "", new List<DatabaseQuery>() { new DatabaseQuery("GroupID", groupId), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) }, "GroupName ASC");
        }
        ...
        public string GetGroupName_byID(string id) {
            ...
            DatabaseQuery dbSelect = dbCall.CallSelectSingle("GroupList", "GroupName", new List<DatabaseQuery>() { new DatabaseQuery("GroupID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
            return dbSelect.Value;
        }

        public string GetOwner(string id) {
            DatabaseQuery dbSelect = dbCall.CallSelectSingle("GroupList", "CreatedBy", new List<DatabaseQuery>() { new DatabaseQuery("GroupID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
            return dbSelect.Value;
        }

        public string GetGroupImg_byID(string id) {
            List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("GroupList", "Image, IsURL", new List<DatabaseQuery>() { new DatabaseQuery("GroupID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
            if (dbSelect.Count > 0) {
        ...
        public void UpdateItem(string id, string groupname, string description, string image, string date, bool isurl, bool isPrivate) {
            ...
            dbCall.CallUpdate("GroupList", updateQuery, new List<DatabaseQuery>() { new DatabaseQuery("GroupID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        }

        public void deleteGroup(string id) {
            dbCall.CallDelete("GroupList", new List<DatabaseQuery>() { new DatabaseQuery("GroupID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
            DeleteDefaults(id);
        }

        public void deleteGroup(Guid id, string groupname) {
            dbCall.CallDelete("GroupList", new List<DatabaseQuery>() { new DatabaseQuery("GroupName", groupname), new DatabaseQuery("GroupID", id.ToString()), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        ...
        private void DeleteDefaults(string id) {
            ...
            Notifications.Notifications noti = new Notifications.Notifications();
