...
public class AppLogIgnore {
    ...
    public void GetEventCommentListOnly() {
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_WebEvents_Ignored", "", new List<DatabaseQuery>() { new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) }, "DatePosted DESC");
        foreach (Dictionary<string, string> row in dbSelect) {
            string ec = HttpUtility.UrlDecode(row["EventComment"]);
    ...
    public void deleteRecord(string id) {
        dbCall.CallDelete("aspnet_WebEvents_Ignored", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
    }

    public void deleteRecords() {
        dbCall.CallDelete("aspnet_WebEvents_Ignored", new List<DatabaseQuery>() { new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
    }
    ...
    public void UpdateRefreshOnError(string id, bool refreshOnError) {
        ...
        dbCall.CallUpdate("aspnet_WebEvents_Ignored", new List<DatabaseQuery>() { new DatabaseQuery("RefreshOnError", _refreshOnError.ToString()) }, new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
    }

    public bool GetRefreshOnError(string id) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("aspnet_WebEvents_Ignored", "RefreshOnError", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        return HelperMethods.ConvertBitToBoolean(dbSelect.Value);
    }

    public void UpdateTimesHit(string id, int count) {
        dbCall.CallUpdate("aspnet_WebEvents_Ignored", new List<DatabaseQuery>() { new DatabaseQuery("TimesHit", count.ToString()) }, new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
    }
