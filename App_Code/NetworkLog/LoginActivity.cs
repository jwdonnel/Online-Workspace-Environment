...
public class LoginActivity
{
    ...
    public LoginActivity_Coll GetActivity(string id) {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_LoginActivity", "", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        foreach (Dictionary<string, string> row in dbSelect) {
            string ipAddress = row["IPAddress"];
            string nameUsed = row["NameUsed"];
            string loginGroup = row["LoginGroup"];
    ...
    public int GetTotalIPAttempts(string ipAddress) {
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_LoginActivity", "ID", new List<DatabaseQuery>() { new DatabaseQuery("IPAddress", ipAddress), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        return dbSelect.Count;
    }

    public void DeleteItem(string id) {
        dbCall.CallDelete("aspnet_LoginActivity", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
    }
