...
public class StartupScripts {
    ...
    public int GetSequence(string script) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("StartupScripts", "Sequence", new List<DatabaseQuery>() { new DatabaseQuery("ScriptPath", script), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        int count = 0;
    ...
    public int GetSequence_ByID(string id) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("StartupScripts", "Sequence", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        int count = 0;
        int.TryParse(dbSelect.Value, out count);
        return count;
    }
    ...
    public string GetScriptPathFromSequence(int sequence) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("StartupScripts", "ScriptPath", new List<DatabaseQuery>() { new DatabaseQuery("Sequence", sequence.ToString()), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }

    public string GetAppliesToFromID(string id) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("StartupScripts", "ApplyTo", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }
    ...
    public List<string> GetScriptIDFromSequence(int sequence) {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("StartupScripts", "ID", new List<DatabaseQuery>() { new DatabaseQuery("Sequence", sequence.ToString()), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
    ...
    public bool CheckIfExists(string scriptPath) {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("StartupScripts", "ScriptPath", new List<DatabaseQuery>() { new DatabaseQuery("ScriptPath", scriptPath), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        foreach (Dictionary<string, string> row in dbSelect) {
    ...
    public bool CheckIfExists(string scriptPath, string applyto) {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("StartupScripts", "ScriptPath", new List<DatabaseQuery>() { new DatabaseQuery("ScriptPath", scriptPath), new DatabaseQuery("ApplyTo", applyto), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        foreach (Dictionary<string, string> row in dbSelect) {
    ...
    public void deleteStartupScript(string id) {
        dbCall.CallDelete("StartupScripts", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        GetScripts();
        int count = 1;
