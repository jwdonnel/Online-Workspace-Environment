...
namespace OpenWSE_Tools.BackgroundServiceDatabaseCalls {
    ...
    public class BackgroundServices : BackgroundServiceCalls {
        ...
        public BackgroundServices_Coll GetBackgroundService(string id) {
            ...
            List<Dictionary<string, string>> dbSelect = dbCall.CallSelect(TableName, "", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        ...
        public List<BackgroundServices_Coll> GetBackgroundServices() {
            ...
            List<Dictionary<string, string>> dbSelect = dbCall.CallSelect(TableName, "", new List<DatabaseQuery>() { new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) }, "Name ASC");
            foreach (Dictionary<string, string> dicVal in dbSelect) {
                string id = dicVal["ID"];
                string name = dicVal["Name"];
                string description = dicVal["Description"];
        ...
        public bool DoesLogInformation(string serviceNamespace) {
            DatabaseQuery dbSelect = dbCall.CallSelectSingle(TableName, "LogInformation", new List<DatabaseQuery>() { new DatabaseQuery("Namespace", serviceNamespace), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
            return HelperMethods.ConvertBitToBoolean(dbSelect.Value);
        }

        public string GetBinFolder(string serviceNamespace) {
            DatabaseQuery dbSelect = dbCall.CallSelectSingle(TableName, "DLL_Location", new List<DatabaseQuery>() { new DatabaseQuery("Namespace", serviceNamespace), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
            return ServerSettings.GetServerMapLocation + "Bin\\" + dbSelect.Value;
        }

        public void UpdateItem(string id, string name, string description, bool logInformation, string username) {
            string logInfo = "0";
            if (logInformation) {
                logInfo = "1";
            }
