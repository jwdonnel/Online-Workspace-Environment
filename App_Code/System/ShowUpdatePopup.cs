...
public class ShowUpdatePopup {
    ...
    public bool isUserShowPopup(string username) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("aspnet_ShowUpdates", "ShowPopup", new List<DatabaseQuery>() { new DatabaseQuery("UserName", username.ToLower()), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
