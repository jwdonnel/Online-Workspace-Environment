                ...
                for (int i = 0; i < appList.Count; i++) {
                    ...
                    if (!string.IsNullOrEmpty(row.AppName)) {
                        if (row.AppName.ToLower().Contains(prefixText) || (row.Description.ToLower().Contains(prefixText) && !string.IsNullOrEmpty(row.Description))) {
                            ...
                            if (!_ss.HideAllAppIcons && !hideIcons && !string.IsNullOrEmpty(row.Icon)) {
                                string iconFile = ServerSettings.GetServerMapLocation + "Standard_Images\\App_Icons\\" + row.Icon;
                                if (File.Exists(iconFile)) {
                                    icon = "<img alt='' src='" + ServerSettings.ResolveUrl("~/Standard_Images/App_Icons/" + row.Icon) + "' />";
                                }
                            }
                ...
                for (int i = 0; i < appList.Length; i++) {
                    try {
                        ...
                        if (row.AppName.ToLower().Contains(prefixText) || (row.Description.ToLower().Contains(prefixText) && !string.IsNullOrEmpty(row.Description))) {
                            ...
                            string[] entry = new string[3];
                            entry[0] = row.AppName;
                            entry[1] = row.AppName + description;
                            entry[2] = "Apps";
