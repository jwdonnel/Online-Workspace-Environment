...
public class CustomTableAppCreator {
    ...
    public string CreateApp(string appName, string categoryid, string description, string picname, bool insertIntoAppList, bool isPrivate, string fileName = "") {
        ...
        appStr.AppendLine("         }");
        appStr.AppendLine("");
        appStr.AppendLine("         ChartType chart_Type = ctv.GetChartTypeFromCustomizations(coll.TableCustomizations);");
        appStr.AppendLine("         string chartColumns = ctv.GetChartColumnsFromCustomizations(coll.TableCustomizations);");
        ...
        appStr.AppendLine("             string chartTitle = ctv.GetChartTitleFromCustomizations(coll.TableCustomizations);");
        ...
        appStr.AppendLine("                 <div class=\"searchwrapper\" style=\"width: 300px;\">");
        ...
        appStr.AppendLine("                         <input id=\"btn_" + fileName + "_viewRecords\" type=\"button\" class=\"input-buttons-create no-margin\" value=\"View Records\" onclick=\"customTables.ViewRecords_MenuClick('" + fileName + "');\" />");
        appStr.AppendLine("                         <div class=\"clear-space\"></div>");
        appStr.AppendLine("                         <input id=\"btn_" + fileName + "_addRecord\" runat=\"server\" type=\"button\" class=\"input-buttons-create no-margin\" value=\"Add Record\" onclick=\"customTables.AddRecord_MenuClick('" + fileName + "');\" />");
        appStr.AppendLine("                     </div>");
        ...
        appStr.AppendLine("                         <select id=\"viewmode-selector-" + fileName + "\" onchange=\"customTables.ChangeViewMode('" + fileName + "')\">");
        ...
        appStr.AppendLine("                         <div class=\"clear-space-five\"></div>");
        appStr.AppendLine("                         <select id=\"font-size-selector-" + fileName + "\" onchange=\"customTables.ChangeFont('" + fileName + "')\">");
        ...
        appStr.AppendLine("                         <div class=\"clear-space\"></div>");
        appStr.AppendLine("                         <input id=\"btnExport-" + fileName + "\" type=\"button\" class=\"input-buttons\" onclick=\"customTables.ExportToExcelAll('" + fileName + "')\" title=\"Select a date to export to an Excel file\" value=\"Export\" />");
        ...
        appStr.AppendLine("                     </div>");
        appStr.AppendLine("                 </li>");
        appStr.AppendLine("                 <li id=\"td_" + fileName + "_sidebar\">");
        appStr.AppendLine("                     <div class=\"section-pad\">");
        appStr.AppendLine("                         <div id=\"sidebar-items-" + fileName + "\">");
        appStr.AppendLine("                             <h3>Month Selector</h3>");
        appStr.AppendLine("                             <div class=\"clear-space-five\"></div>");
        appStr.AppendLine("                             <div class=\"pad-right\"><small><b class=\"pad-right-sml\">Note:</b>These dates are taken from the timestamp of each item created.</small></div>");
        appStr.AppendLine("                             <div class=\"clear-space\"></div>");
        ...
        if (insertIntoAppList)
            _app.CreateItem(appId, appName, "Custom_Tables/" + fileName + ".ascx", picname, "1", "1", OpenWSE.Core.Licensing.CheckLicense.SiteName + " | " + ServerSettings.ServerDateTime.Year, description, "app-main", categoryid, "450", "500", false, true, "~/ExternalAppHolder.aspx?appId=" + appId, "", "", false, "1", isPrivate, true, string.Empty, "#FFFFFF");
    ...
    public string CreateSummaryApp(string appName, string categoryid, string description, string picname, bool insertIntoAppList, bool isPrivate, string fileName = "") {
        StringBuilder appStr = new StringBuilder();
        if (string.IsNullOrEmpty(fileName))
            fileName = HelperMethods.RandomString(10);
        ...
        appStr.AppendLine("         System.Security.Principal.IIdentity userId = Page.User.Identity;");
        appStr.AppendLine("         _apps = new App(userId.Name);");
        appStr.AppendLine("         CustomTableSummary cts = new CustomTableSummary(userId.Name);");
        appStr.AppendLine("         CustomSummary_Coll coll = cts.GetSummaryInfoByAppId(app_id);");
        appStr.AppendLine("         lbl_Title_" + fileName + ".Text = coll.SummaryName;");
        appStr.AppendLine("");
        appStr.AppendLine("         if (!_ss.HideAllAppIcons) {");
        appStr.AppendLine("             img_Title_" + fileName + ".Visible = true;");
        appStr.AppendLine("             string clImg = _apps.GetAppIconName(app_id);");
        ...
        appStr.AppendLine("");
        appStr.AppendLine("         _appInitializer = new OpenWSE_Tools.Apps.AppInitializer(app_id, Page.User.Identity.Name, Page, \"" + applyTo + "\");");
        appStr.AppendLine("         _appInitializer.LoadScripts_JS(true, \"summaryTables.Load('" + fileName + "');\");");
        appStr.AppendLine("     }");
        appStr.AppendLine("</script>");


        // HTML Code
        appStr.AppendLine("<div class=\"custom-table-summary-timermarker\">");
        #region custom-table-tableView-holder
        appStr.AppendLine("    <div id=\"pnl_" + fileName + "_summaryView\" class=\"custom-table-tableView-summary-holder\" data-viewmode=\"\">");
        appStr.AppendLine("         <div class=\"pad-all app-title-bg-color\">");
        appStr.AppendLine("             <div class=\"float-left\">");
        appStr.AppendLine("                 <asp:Image ID=\"img_Title_" + fileName + "\" runat=\"server\" CssClass=\"app-img-titlebar\" />");
        appStr.AppendLine("                 <asp:Label ID=\"lbl_Title_" + fileName + "\" runat=\"server\" Text=\"\" CssClass=\"app-text-titlebar\"></asp:Label>");
        appStr.AppendLine("             </div>");
        ...
        appStr.AppendLine("         </div>");
        appStr.AppendLine("         <div class=\"clear-space\"></div>");
        appStr.AppendLine("         <div class=\"float-left pad-left\">");
        appStr.AppendLine("             <span class=\"pad-right-sml font-bold\">View Mode</span>");
        appStr.AppendLine("             <select id=\"viewmode-selector-" + fileName + "\" onchange=\"summaryTables.ChangeViewMode('" + fileName + "')\">");
        appStr.AppendLine("                 <option value=\"desktop\">Desktop</option>");
        appStr.AppendLine("                 <option value=\"mobile\">Mobile</option>");
        appStr.AppendLine("             </select>");
        appStr.AppendLine("         </div>");
        appStr.AppendLine("         <a href=\"#\" class=\"float-right img-refresh margin-right margin-top-sml\" onclick=\"summaryTables.Refresh('" + fileName + "');return false;\" title=\"Refresh\"></a>");
        appStr.AppendLine("         <div class=\"float-right pad-right-big\">");
        appStr.AppendLine("             <span class=\"pad-right-sml font-bold\">Font Size</span>");
        appStr.AppendLine("             <select id=\"font-size-selector-" + fileName + "\" onchange=\"summaryTables.ChangeFont('" + fileName + "')\">");
        appStr.AppendLine("                 <option value=\"x-small\">X-Small</option>");
        appStr.AppendLine("                 <option value=\"small\" selected=\"selected\">Small</option>");
        appStr.AppendLine("                 <option value=\"medium\">Medium</option>");
        appStr.AppendLine("                 <option value=\"large\">Large</option>");
