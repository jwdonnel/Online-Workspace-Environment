...
public class CustomTable_Coll {
    ...
    public CustomTable_Coll(string id, string tableID, string tableName, string description, string appID, string columnData, string notifyUsers, string usersAllowedToEdit, string tableCustomizations, string updatedBy, string dateUpdated) {
        _id = id;
...
public class CustomTableViewer {
    ...
    public void AddItem(string tableID, string tableName, string description, string appId, string columnData, string updatedBy, bool notifyUsers, string usersAllowToEdit, string tableCustomizations) {
        string _nu = "0";
        if (notifyUsers) {
            _nu = "1";
        }
    ...
    public void DeleteRowByID(string id, string appID) {
        ...
        bool tableDeleted = dbCall.CallDelete("CustomTables", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
    ...
    public CustomTable_Coll GetTableInfoByAppId(string appID) {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("CustomTables", "", new List<DatabaseQuery>() { new DatabaseQuery("AppID", appID), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        foreach (Dictionary<string, string> row in dbSelect) {
            ...
            cInfo = new CustomTable_Coll(id, tableID, tableName, description, appID, columnData, notifyUsers, usersAllowedToEdit, tableCustomizations, updatedBy, dateUpdated);
            break;
        }
        return cInfo;
    }
    public CustomTable_Coll GetTableInfoByTableId(string tableID) {
        CustomTable_Coll cInfo = new CustomTable_Coll();
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("CustomTables", "", new List<DatabaseQuery>() { new DatabaseQuery("TableID", tableID), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        foreach (Dictionary<string, string> row in dbSelect) {
            string id = row["ID"];
            string tableName = row["TableName"];
            string description = row["Description"];
    ...
    public string GetAppIdByTableID(string TableID) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("CustomTables", "AppID", new List<DatabaseQuery>() { new DatabaseQuery("TableID", TableID), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }
    public string GetTableIDByAppID(string AppID) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("CustomTables", "TableID", new List<DatabaseQuery>() { new DatabaseQuery("AppID", AppID), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }
    public string GetTableIDByID(string id) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("CustomTables", "TableID", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }
    public string GetTableNameByAppID(string AppID) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("CustomTables", "TableName", new List<DatabaseQuery>() { new DatabaseQuery("AppID", AppID), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }
    public string GetTableNameByTableID(string tableId) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("CustomTables", "TableName", new List<DatabaseQuery>() { new DatabaseQuery("TableID", tableId), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }
    public string GetUpdatedByByID(string id) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("CustomTables", "UpdatedBy", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }
    public string GetAppIDByID(string id) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("CustomTables", "AppID", new List<DatabaseQuery>() { new DatabaseQuery("ID", id), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }
    ...
    public void BuildEntriesAll(string sortCol = "", string sortDir = "") {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("CustomTables", "", query, sortCol + " " + sortDir);
        foreach (Dictionary<string, string> row in dbSelect) {
            string id = row["ID"];
            string tableID = row["TableID"];
            string tableName = row["TableName"];
            ...
            string updatedBy = row["UpdatedBy"];
            string dateUpdated = row["DateUpdated"];
            CustomTable_Coll coll = new CustomTable_Coll(id, tableID, tableName, description, appID, columnData, notifyUsers, usersAllowedToEdit, tableCustomizations, updatedBy, dateUpdated);
            _coll.Add(coll);
        }
    }
    public void BuildEntriesForUser(string sortCol = "", string sortDir = "") {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("CustomTables", "", new List<DatabaseQuery>() { new DatabaseQuery("UpdatedBy", _userName), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) }, sortCol + " " + sortDir);
        foreach (Dictionary<string, string> row in dbSelect) {
            string id = row["ID"];
            string tableID = row["TableID"];
            string tableName = row["TableName"];
    ...
    public void UpdateRow(string tableID, string tableName, string description, string columnData, bool notifyUsers, string usersAllowToEdit, string tableCustomizations) {
        string _nu = "0";
        if (notifyUsers) {
            _nu = "1";
        }
