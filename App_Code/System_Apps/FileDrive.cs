...
public class FileDrive {
    ...
    public void GetAllFolders(string groupname) {
        ...
        List<Dictionary<string, string>> dataTable = dbCall.CallSelect("FileDriveFolders", "", new List<DatabaseQuery>() { new DatabaseQuery("GroupName", groupname), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
    ...
    public string GetFolderForMove(Guid id) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("FileDriveDocuments", "Folder", new List<DatabaseQuery>() { new DatabaseQuery("ID", id.ToString()), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }

    public string GetFolderbyID(Guid id) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("FileDriveFolders", "Folder", new List<DatabaseQuery>() { new DatabaseQuery("ID", id.ToString()), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }

    public string GetFilenamebyID(Guid id) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("FileDriveDocuments", "FileName", new List<DatabaseQuery>() { new DatabaseQuery("ID", id.ToString()), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        return dbSelect.Value;
    }
    ...
    public void GetFilesByID(Guid id) {
        ...
        List<Dictionary<string, string>> dataTable = dbCall.CallSelect("FileDriveDocuments", "", new List<DatabaseQuery>() { new DatabaseQuery("ID", id.ToString()), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        foreach (Dictionary<string, string> row in dataTable) {
            string fn = row["FileName"];
            string fe = row["FileExtension"];
            string fs = row["FileSize"];
    ...
    public void GetFilesByOwner(string owner, string groupname) {
        ...
        List<Dictionary<string, string>> dataTable = dbCall.CallSelect("FileDriveDocuments", "", new List<DatabaseQuery>() { new DatabaseQuery("Owner", owner), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        foreach (Dictionary<string, string> row in dataTable) {
            var id = Guid.Parse(row["ID"]);
            string fn = row["FileName"];
            string fe = row["FileExtension"];
    ...
    public void GetPersonalFiles(string folder, string group) {
        ...
        List<Dictionary<string, string>> dataTable = dbCall.CallSelect("FileDriveDocuments", "", new List<DatabaseQuery>() { new DatabaseQuery("Folder", folder), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        foreach (Dictionary<string, string> row in dataTable) {
            var id = Guid.Parse(row["ID"]);
            string fn = row["FileName"];
            string fe = row["FileExtension"];
    ...
    public void GetFilesByFilename(string filename, string groupname) {
        ...
        List<Dictionary<string, string>> dataTable = dbCall.CallSelect("FileDriveDocuments", "", new List<DatabaseQuery>() { new DatabaseQuery("FileName", filename), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        foreach (Dictionary<string, string> row in dataTable) {
            var id = Guid.Parse(row["ID"]);
            string fn = row["FileName"];
            string fe = row["FileExtension"];
    ...
    public void GetFilesByExtension(string ext, string groupname) {
        ...
        List<Dictionary<string, string>> dataTable = dbCall.CallSelect("FileDriveDocuments", "", new List<DatabaseQuery>() { new DatabaseQuery("FileExtension", ext), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        foreach (Dictionary<string, string> row in dataTable) {
            var id = Guid.Parse(row["ID"]);
            string fn = row["FileName"];
            string fe = row["FileExtension"];
    ...
    public string GetFileNamePath(Guid id) {
        ...
        List<Dictionary<string, string>> dataTable = dbCall.CallSelect("FileDriveDocuments", "FileName, FilePath", new List<DatabaseQuery>() { new DatabaseQuery("ID", id.ToString()), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
        foreach (Dictionary<string, string> row in dataTable) {
            string fn = row["FileName"];
            string fp = row["FilePath"];
    ...
    public bool CheckFolderExists(string folder, string groupname) {
        ...
        if (string.IsNullOrEmpty(groupname)) {
            List<Dictionary<string, string>> dataTable = dbCall.CallSelect("FileDriveFolders", "ID", new List<DatabaseQuery>() { new DatabaseQuery("Folder", folder), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
            ...
            foreach (string gr in groupList) {
                List<Dictionary<string, string>> dataTable = dbCall.CallSelect("FileDriveFolders", "ID", new List<DatabaseQuery>() { new DatabaseQuery("Folder", folder), new DatabaseQuery("GroupName", gr), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
                if (dataTable.Count > 0) {
                    exists = true;
                    break;
                }
    ...
    public void deleteFile(Guid id) {
        dbCall.CallDelete("FileDriveDocuments", new List<DatabaseQuery>() { new DatabaseQuery("ID", id.ToString()), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
    }
    ...
    public void deleteFolder(Guid id, string groupname) {
        ...
        dbCall.CallDelete("FileDriveFolders", new List<DatabaseQuery>() { new DatabaseQuery("ID", id.ToString()), new DatabaseQuery("ApplicationId", ServerSettings.ApplicationID) });
    }
