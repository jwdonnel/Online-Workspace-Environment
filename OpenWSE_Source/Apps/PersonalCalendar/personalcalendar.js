...
prm.add_beginRequest(function (sender, args) {
    if ($("#NewCalEvent-element").css("display") == "block") {
        temp_startDate_pc = $("#app-personalcalendar").find("#tb_newevent_start_pc").val();
        temp_endDate_pc = $("#app-personalcalendar").find("#tb_newevent_end_pc").val();
        temp_startTime_pc = $("#app-personalcalendar").find("#dd_newevent_starttime_pc").val();
        temp_endTime_pc = $("#app-personalcalendar").find("#dd_newevent_endtime_pc").val();
        temp_startampm_pc = $("#app-personalcalendar").find("#dd_newevent_starttime_pc_ampm").val();
        temp_endampm_pc = $("#app-personalcalendar").find("#dd_newevent_endtime_pc_ampm").val();
        temp_reason_pc = $("#app-personalcalendar").find("#tb_newevent_title_pc").val();
        temp_description_pc = $("#app-personalcalendar").find("#tb_newevent_desc_pc").val();
        temp_hours_pc = $("#app-personalcalendar").find("#tb_hours").val();
    }
});

function RestorePostBack_pc() {
    if ($("#NewCalEvent-element").css("display") == "block") {
        $("#app-personalcalendar").find("#tb_newevent_start_pc").datepicker({
            defaultDate: "+1w",
            changeMonth: true,
            changeYear: true,
            numberOfMonths: 2,
            onClose: function (selectedDate) {
                $("#app-personalcalendar").find("#tb_newevent_end_pc").datepicker("option", "minDate", selectedDate);
            }
        });
        $("#app-personalcalendar").find("#tb_newevent_end_pc").datepicker({
            defaultDate: "+1w",
            changeMonth: true,
            changeYear: true,
            numberOfMonths: 2
        ...
        if (temp_startTime_pc != "") {
            $("#app-personalcalendar").find("#tb_newevent_start_pc").val(temp_startDate_pc);
            $("#app-personalcalendar").find("#tb_newevent_end_pc").val(temp_endDate_pc);
            $("#app-personalcalendar").find("#dd_newevent_starttime_pc").val(temp_startTime_pc);
            $("#app-personalcalendar").find("#dd_newevent_endtime_pc").val(temp_endTime_pc);
            $("#app-personalcalendar").find("#dd_newevent_starttime_pc_ampm").val(temp_startampm_pc);
            $("#app-personalcalendar").find("#dd_newevent_endtime_pc_ampm").val(temp_endampm_pc);
            $("#app-personalcalendar").find("#tb_newevent_title_pc").val(temp_reason_pc);
            $("#app-personalcalendar").find("#tb_newevent_desc_pc").val(temp_description_pc);
            $("#app-personalcalendar").find("#tb_hours").val(temp_hours_pc);

            $("#app-personalcalendar").find("#tb_newevent_end_pc").datepicker("option", "minDate", temp_startDate_pc);
...
$(document.body).on("click", "#app-personalcalendar .maximize-button-app", function () {
    if ($("#personalcalendar-load").length > 0) {
        onCalendarLoad_pc();
    }
});

$(document.body).on("dblclick", "#app-personalcalendar .app-head-dblclick", function () {
    if ($("#personalcalendar-load").length > 0) {
        onCalendarLoad_pc();
    }
});
...
function onCalendarLoad_pc() {
    ...
    $("#app-personalcalendar").resizable({
...
function buildCalendar_PC(m, y) {
    ...
    var h = $("#app-personalcalendar").height() - $("#app-personalcalendar .app-head").height();
    table_Days_Height_pc = h - ($("#app-personalcalendar").find(".pc-menu-bar").height() + $("#pCal-titlebar").height());
...
function GetMonthInfo_PC(month, year) {
    if (!reloadingSchedule_pc) {
        ...
        selected_m_pc = month;
        selected_y_pc = year;
        viewtype = $("#app-personalcalendar").find("#hf_viewtype_vacationentry").val();
                ...
                if (response.length > 0) {
                    ...
                    for (var i = 0; i < response.length; i++) {
                        if (response[i].length == 6) {
                            ...
                            var tdWidth = $("#app-personalcalendar").find("#" + startDate).width();

                            var bufferLen = len;
                            if (len > 1) {
                                var $nextTd = $("#app-personalcalendar").find("#" + startDate).next("td");
                            ...
                            $("#app-personalcalendar").find("#" + startDate).append(eventDiv);
                        }
                    }
...
function SetMarginEvents_pc() {
    ...
    var length_temp = 0;
    var bufferMargin = 2;
    $("#app-personalcalendar").find(".table-Days").each(function (index) {
...
function onNextClick_PC() {
    ...
    var m = parseInt($("#app-personalcalendar").find("#dd_monthSelect").val());
    var y = parseInt($("#app-personalcalendar").find("#dd_yearSelect").val());
    if (m == 11) {
        y = y + 1;
        m = 0;
    }
...
function DisplayEvents_pc() {
    $("#app-personalcalendar").find(".table-Days td").each(function (index) {
...
function AddNewCalendarEvent_pc(startDate, edit) {
    if ($("#NewCalEvent-element").css("display") != "block") {
        ...
        $("#app-personalcalendar").find("#tb_newevent_end_pc").datepicker("option", "minDate", startDate);

        if ($("#hf_editmode-pc").val() == "" || !openWSE.ConvertBitToBoolean($("#hf_editmode-pc").val())) {
            $("#app-personalcalendar").find("#tb_newevent_start_pc").val(startDate);
            $("#app-personalcalendar").find("#tb_newevent_end_pc").val(startDate);
            $("#app-personalcalendar").find("#tb_newevent_title_pc").val("Event for " + startDate);
            $("#app-personalcalendar").find("#btn_createnewevent_pc").val("Create");
        }
        else {
            $("#app-personalcalendar").find("#btn_createnewevent_pc").val("Update");
        }
        ...
        $("#app-personalcalendar").find("#tb_newevent_title_pc").val("");
        $("#app-personalcalendar").find("#tb_newevent_desc_pc").val("");
        $("#app-personalcalendar").find("#tb_newevent_start_pc").val("");
        $("#app-personalcalendar").find("#tb_newevent_end_pc").val("");
        $("#app-personalcalendar").find("#btn_createnewevent_pc").show();
    ...
    if ($("#ViewCalEvent-element").css("display") != "block") {
        $.ajax({
            ...
            success: function (data) {
                ...
                if (response != "") {
                    _id_pc = id;
                    $("#app-personalcalendar").find("#eventholderCal_modal").html(data.d);
        ...
        setTimeout(function () {
            _id_pc = "";
            $("#hf_editmode-pc").val("false");
            $("#app-personalcalendar").find("#eventholderCal_modal").html("");
...
function EditCalendarEvent_pc(title, desc, color, startDate, endDate) {
    ...
    $("#app-personalcalendar").find("#tb_newevent_start_pc").val(sdate);
    $("#app-personalcalendar").find("#tb_newevent_end_pc").val(edate);
    $("#app-personalcalendar").find("#dd_newevent_starttime_pc").val(dstart[1].replace(":00", ""));
    $("#app-personalcalendar").find("#dd_newevent_endtime_pc").val(dend[1].replace(":00", ""));
    $("#app-personalcalendar").find("#dd_newevent_starttime_pc_ampm").val(dstart[2]);
    $("#app-personalcalendar").find("#dd_newevent_endtime_pc_ampm").val(dend[2]);
    $("#app-personalcalendar").find("#tb_newevent_title_pc").val(title);
    $("#app-personalcalendar").find("#tb_newevent_desc_pc").val(desc);

    $("#app-personalcalendar").find(".calendar-bg-selectors").each(function (index) {
...
function btn_createnewevent_pc_Clicked() {
    var title = $("#app-personalcalendar").find("#tb_newevent_title_pc").val();
    var desc = $("#app-personalcalendar").find("#tb_newevent_desc_pc").val();
    var start = $("#app-personalcalendar").find("#tb_newevent_start_pc").val() + " " + $("#app-personalcalendar").find("#dd_newevent_starttime_pc").val() + " " + $("#app-personalcalendar").find("#dd_newevent_starttime_pc_ampm").val();
    var end = $("#app-personalcalendar").find("#tb_newevent_end_pc").val() + " " + $("#app-personalcalendar").find("#dd_newevent_endtime_pc").val() + " " + $("#app-personalcalendar").find("#dd_newevent_endtime_pc_ampm").val();

    if (($.trim(title) == "") || ($.trim(start) == "") || ($.trim(end) == "")) {
        $("#app-personalcalendar").find("#errormessagepc").html("Must fill out Title, Start Date, and End Date.");
        setTimeout(function () {
            $("#app-personalcalendar").find("#errormessagepc").html("");
        }, 3000);
    }
    else {
...
$(document.body).on("click", "#app-personalcalendar .calendar-bg-selectors", function () {
    var $this = $(this);
    $("#app-personalcalendar").find(".calendar-bg-selectors").each(function (index) {
        if ($(this).hasClass("selected")) {
            $(this).removeClass("selected");
...
function LoadingPersonalCalendar(message) {
    $("#update-element-pc").remove();
    var x = "<div id='update-element-pc'><div class='update-element-overlay' style='position: absolute!important'><div class='update-element-align' style='position: absolute!important'>";
    x += "<div class='update-element-modal'>" + openWSE.loadingImg + "<h3 class='inline-block'>";
    x += message + "</h3></div></div></div></div>";
    $("#personalcalendar-load").append(x);
    $("#update-element-pc").show();
}

var eleClass;
$(document.body).on("mouseenter", "#app-personalcalendar .calendar-event", function () {
    eleClass = "";
...
$(document.body).on("mouseleave", "#app-personalcalendar .calendar-event", function () {
