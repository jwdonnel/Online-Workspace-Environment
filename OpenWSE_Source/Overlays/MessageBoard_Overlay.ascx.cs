...
public partial class Overlays_MessageBoard_Overlay : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        _overlayInit = new OverlayInitializer(userId.Name, "Overlays/MessageBoard_Overlay.ascx");
        if (_overlayInit.TryLoadOverlay)
        {
            _username = userId.Name;
            _member = new MemberDatabase(_username);

            AppInitializer _appInitializer = new AppInitializer("app-messageboard", _username, Page);
            _appInitializer.LoadScripts_JS(true, "StartMessageBoard();GetMessageBoardOverlayPosts()");
        }
