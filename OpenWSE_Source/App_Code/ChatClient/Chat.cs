...
public class Chat
{
    ...
    public Chat(bool getvalues)
    {
        if (getvalues)
        {
            ...
            List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_Chat", "", new List<DatabaseQuery>() { new DatabaseQuery("SessionDate", DateTime.Now.ToShortDateString()) }, "Date ASC");
    ...
    public void DeleteChatLog() {
        dbCall.CallDelete("aspnet_Chat", null);
    }

    public void DeleteUserChats(string userName) {
        dbCall.CallDelete("aspnet_Chat", new List<DatabaseQuery>() { new DatabaseQuery("tUser", userName) });
        dbCall.CallDelete("aspnet_Chat", new List<DatabaseQuery>() { new DatabaseQuery("sUser", userName) });
    }
    ...
    public List<ChatColl> GetAllChats
    {
        get {
            ...
            List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_Chat", "", null, "Date ASC");
            foreach (Dictionary<string, string> row in dbSelect) {
                string id = row["ID"];
                string tuser = row["tUser"].ToLower();
                string suser = row["sUser"].ToLower();
    ...
    {
        List<ChatColl> temp = new List<ChatColl>();
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("aspnet_Chat", "", new List<DatabaseQuery>() { new DatabaseQuery("SessionDate", dateSelected) }, "Date ASC");
        foreach (Dictionary<string, string> row in dbSelect) {
            string id = row["ID"];
            string tuser = row["tUser"].ToLower();
            string suser = row["sUser"].ToLower();
    ...
    {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("aspnet_Chat", "tUser", new List<DatabaseQuery>() { new DatabaseQuery("ID", id) });
        return dbSelect.Value;
    }

    public string GetsUser(string id)
    {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("aspnet_Chat", "sUser", new List<DatabaseQuery>() { new DatabaseQuery("ID", id) });
        return dbSelect.Value;
    }
    ...
    public DateTime GetDate(string id)
    {
        ...
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("aspnet_Chat", "Date", new List<DatabaseQuery>() { new DatabaseQuery("ID", id) });
    ...
    private void CheckIfNextDay()
    {
        if (_dataTable.Count > 0)
        {
            ...
            int totalTime = DateTime.Now.Subtract(date).Days;
            if (totalTime > 0)
                _dataTable.Clear();
        }
    }
