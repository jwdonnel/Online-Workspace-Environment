...
public class dbImportAppCreator
{
    ...
    public string CreateApp(string appName, string categoryid, string description, string picname, bool isPrivate, bool insertIntoAppList = true, string fileName = "")
    {
        ...
        if ((insertIntoAppList) && (!string.IsNullOrEmpty(appName)))
            _app.CreateItem(appId, appName, "Database_Imports/" + fileName + ".ascx", picname, "1", "1", OpenWSE.Core.Licensing.CheckLicense.SiteName + " | " + DateTime.Now.Year, description, "app-main", categoryid, "500", "700", false, true, "~/ExternalAppHolder.aspx?appId=" + appId, "", "", false, true, false, "1", isPrivate);
