...
public class CustomTableAppCreator
{
    ...
    public string CreateApp(bool showSideBar, string appName, string categoryid, string description, string picname, bool insertIntoAppList, bool isPrivate, string fileName = "")
    {
        ...
        if (insertIntoAppList)
            _app.CreateItem(appId, appName, "Custom_Tables/" + fileName + ".ascx", picname, "1", "1", OpenWSE.Core.Licensing.CheckLicense.SiteName + " | " + DateTime.Now.Year, description, "app-main", categoryid, "700", "900", false, true, "~/ExternalAppHolder.aspx?appId=" + appId, "", "", false, true, false, "1", isPrivate);
