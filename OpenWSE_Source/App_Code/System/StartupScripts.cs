...
public class StartupScripts {
    ...
    public StartupScripts(bool getvalues) {
        if (getvalues) {
            GetScripts();
        }
    }
    ...
    public int GetSequence(string script) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("StartupScripts", "Sequence", new List<DatabaseQuery>() { new DatabaseQuery("ScriptPath", script) });
        int count = 0;
    ...
    public int GetSequence_ByID(string id) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("StartupScripts", "Sequence", new List<DatabaseQuery>() { new DatabaseQuery("ID", id) });
        int count = 0;
        int.TryParse(dbSelect.Value, out count);
        return count;
    }
    ...
    public string GetScriptPathFromSequence(int sequence) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("StartupScripts", "ScriptPath", new List<DatabaseQuery>() { new DatabaseQuery("Sequence", sequence.ToString()) });
        return dbSelect.Value;
    }

    public string GetAppliesToFromID(string id) {
        DatabaseQuery dbSelect = dbCall.CallSelectSingle("StartupScripts", "ApplyTo", new List<DatabaseQuery>() { new DatabaseQuery("ID", id) });
        return dbSelect.Value;
    }
    ...
    public List<string> GetScriptIDFromSequence(int sequence) {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("StartupScripts", "ID", new List<DatabaseQuery>() { new DatabaseQuery("Sequence", sequence.ToString()) });
    ...
    public bool CheckIfExists(string scriptPath) {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("StartupScripts", "ScriptPath", new List<DatabaseQuery>() { new DatabaseQuery("ScriptPath", scriptPath) });
        foreach (Dictionary<string, string> row in dbSelect) {
    ...
    public bool CheckIfExists(string scriptPath, string applyto) {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("StartupScripts", "ScriptPath", new List<DatabaseQuery>() { new DatabaseQuery("ScriptPath", scriptPath), new DatabaseQuery("ApplyTo", applyto) });
        foreach (Dictionary<string, string> row in dbSelect) {
    ...
    public void deleteStartupScript(string id) {
        dbCall.CallDelete("StartupScripts", new List<DatabaseQuery>() { new DatabaseQuery("ID", id) });
        GetScripts();
        int count = 1;
    ...
    public void GetScripts() {
        ...
        List<Dictionary<string, string>> dbSelect = dbCall.CallSelect("StartupScripts", "", null, "Sequence ASC");
        foreach (Dictionary<string, string> row in dbSelect) {
